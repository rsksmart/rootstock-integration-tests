...
describe.skip('Calling registerFastBridgeBtcTransaction after iris', () => {
    ...
    it('should return value transferred when calling registerFastBridgeBtcTransaction method', async () => {
        try {
            ...
            expect(Number(await rskTxHelper.getBalance(liquidityBridgeContract._address))).to.equal(
                0
            );
            ...
            const weisToTransfer = Number(
                btcEthUnitConverter.btcToWeis(BTC_BALANCE_TO_TRANSFER_IN_BTC)
            );
            ...
            const data = await fundAddressAndGetData(
                btcTxHelper,
                flyoverFedAddress,
                BTC_BALANCE_TO_TRANSFER_IN_BTC,
                AMOUNT_FOR_FUNDER_IN_BTC
            );
            ...
            const currentRskBalance = await rskTxHelper.getBalance(
                liquidityBridgeContract._address
            );
