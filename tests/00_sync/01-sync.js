...
describe('Federators sync', () => {
    it('should sync all rsk federator nodes when one of them manually mines', async () => {
        try {
            ...
            expect(Runners.hosts.federates.length).to.be.greaterThan(
                0,
                'Federates array cannot be empty'
            );
            ...
            const federatorsLatestBlocks = await rskUtils.mineAndSync(
                rskTransactionHelpers,
                blocksToMine
            );
            const allFederatorsAreSynched = federatorsLatestBlocks.every(
                (blockNumber) => blockNumber === blocksToMine
            );
