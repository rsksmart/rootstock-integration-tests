...
describe.skip('Executing registerFastBridgeBtcTransaction post hop - sending same tx without witness twice', () => {
    ...
    it(`should execute first tx successfully and fail executing second tx due to hash already used when calling registerFastBridgeBtcTransaction sending same tx twice`, async () => {
        try {
            ...
            const userBtcRefundAddressBytes = ensure0x(
                btcTxHelper.decodeBase58Address(userBtcRefundAddress)
            );
            const liquidityProviderBtcAddress = (await btcTxHelper.generateBtcAddress('legacy'))
                .address;
            ...
            const flyoverRedeemScript = redeemScriptParser.getFlyoverRedeemScript(
                powpegRedeemScript,
                derivationHash.substring(2)
            );
            const flyoverFedAddress = redeemScriptParser.getAddressFromRedeemScript(
                'REGTEST',
                flyoverRedeemScript
            );
            ...
            const checkFunction = (result) => {
                ...
                const expectedResultSatoshis = Number(
                    btcEthUnitConverter.btcToSatoshis(EXPECTED_AMOUNT_IN_BTC)
                );
                const expectedResultInWeis = Number(
                    btcEthUnitConverter.satoshisToWeis(expectedResultSatoshis)
                );
            ...
            const currentLbcBalance = Number(
                await rskTxHelper.getBalance(liquidityBridgeContract._address)
            );
            ...
            const checkFunction2 = (result) => {
                ...
                expect(UNPROCESSABLE_TX_ALREADY_PROCESSED_ERROR).to.be.equals(
                    resultValueFromSecondTx
                );
            };
