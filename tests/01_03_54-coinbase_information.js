    ...
    it('should work when calling Bridge coinbase related methods', async () => {
        try {
            ...
            await rskUtils.sendFromCow(
                rskTxHelper,
                rskTxSenderAddress,
                Number(btcEthUnitConverter.btcToWeis(0.1))
            );
            ...
            const check = (resultSoFar, currentAttempts) => {
                console.log(
                    `Attempting to get the btc block coinbase information in the bridge for hash: ${hash}, attempt: ${currentAttempts}.`
                );
                return resultSoFar;
            };

            const { result: hasBtcBlockCoinbaseInformation } = await retryWithCheck(
                hasBtcBlockCoinbaseTransactionInformationMethod,
                check
            );
