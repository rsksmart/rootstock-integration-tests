...
var assertKeyControl = (federates) => (begin, end, amountToTransferInWeis) => {
    var rskClients = federates.slice(begin, end).map((federate) => rsk.getClient(federate.host));
    var rskClient = rskClients[0];

    var federateAddresses = federates
        .slice(begin, end)
        .map((federate) =>
            rskClient.rsk.utils.publicKeyToAddress(
                bitcoin.keys.publicKeyToUncompressed(removePrefix0x(federate.publicKeys.rsk))
            )
        );
    ...
    if (amountToTransferInWeis != null && amountToTransferInWeis > 0) {
        for (const federateAddress of federateAddresses) {
            transfer = transfer
    ...
    return rskUtils
        .mineAndSync(getRskTransactionHelpers())
        .then(() => transfer)
...
var checkUpdateBridge = async (rskClients, federateAddresses, fedIndex) => {
    ...
    const bridge = await getBridge(client);
    ...
    return Promise.resolve()
        ...
        .then((txCount) => {
            ...
            return Promise.all(txsPromise).then((txs) =>
                txs.reduce((found, tx) => {
                    return (
                        found ||
        ...
        .then((found) => {
            expect(found, `Federate node at index ${fedIndex} not able to update the bridge`).to.be
                .true;
        })
