 ...
 * @returns {void}
 */
const assertBitcoinBalance =
    (btcClient, rskClient, pegClient) => (btcAddress, expectedBalance, message) => {
...
const assertLock = (btcClient, rskClient, pegClient) => (fromAddresses, outputs, options) => {
    ...
    let initialFederationBalances;
    let initialBtcBalance;
    let initialRskBalance;
    let expectedFederationBalances;
    const federationAddresses = Object.keys(
        outputs.reduce((ad, output) => ({ ...ad, [output.address]: true }), {})
    );
    const amount = outputs.reduce((t, o) => t + o.amount, 0);

    return Promise.resolve()
        .then(() => btcClient.getAddressBalance(federationAddresses))
        .then((btcBalances) => {
        ...
        .then(() => btcClient.getAddressBalance(fromAddresses.btc))
        .then((btcBalances) => {
        ...
        .then(() =>
            isMultisig(fromAddresses)
                ? btcClient.sendFromMultisigTo(fromAddresses, outputs, BTC_TX_FEE, 1)
                : btcClient.sendFromTo(fromAddresses.btc, outputs, BTC_TX_FEE, 1)
        )
        .then(() => mineAndUpdate(btcClient, rskClient, options.shouldMineAndUpdatePrematurely))
        .then(() => btcClient.generate(3))
        .then(() =>
            assertBitcoinBalance(btcClient, rskClient, pegClient)(
                fromAddresses.btc,
                initialBtcBalance - amount - BTC_TX_FEE,
                'Lock BTC debit'
            )
        )
        .then(() => {
            ...
            for (const address of federationAddresses) {
                result = result.then(() =>
                    assertBitcoinBalance(btcClient, rskClient, pegClient)(
                        address,
                        expectedFederationBalances[address],
                        `Lock BTC federation ${address} credit`
                    )
                );
            }
            return result;
        ...
        .then(async () => {
            if (!isMultisig(fromAddresses)) {
                ...
                const currentRskBalanceInSatoshis = rsk.weisToSatoshis(currentRskBalance);
                if (options.fails) {
                    expect(currentRskBalanceInSatoshis, 'Wrong RSK balance').to.equal(
                        initialRskBalance
                    );
                } else {
                    expect(currentRskBalanceInSatoshis, 'Wrong RSK balance').to.equal(
                        Number(initialRskBalance) + Number(amount)
                    );
                }
            }
        });
...
const mineAndUpdate = (btcClient, rskClient, doIt) => {
    let result = Promise.resolve();
    if (doIt) {
        result = result
...
const isMultisig = (addr) => addr.info && addr.info.type == 'multisig';

const assertCallToPegoutBatchingBridgeMethods =
    (rskClient) => async (expectedCount, expectedNextPegoutCreationBlockNumber) => {
        const count = await rskClient.rsk.bridge.methods.getQueuedPegoutsCount().call();
        expect(Number(count)).to.equal(expectedCount);

        const estimatedFees = await rskClient.rsk.bridge.methods
            .getEstimatedFeesForNextPegOutEvent()
            .call();
        // Using Arithmetic Sequence To Calculate The Estimated Fee
        const expectedEstimatedFee =
            expectedCount > 0
                ? MAX_ESTIMATED_FEE_PER_PEGOUT + (expectedCount - 1) * FEE_DIFFERENCE_PER_PEGOUT
                : 0;
        ...
        const nextPegoutCreationBlockNumber = await rskClient.rsk.bridge.methods
            ...
            .call();
        expect(Number(nextPegoutCreationBlockNumber)).to.equal(
            expectedNextPegoutCreationBlockNumber
        );
    };
