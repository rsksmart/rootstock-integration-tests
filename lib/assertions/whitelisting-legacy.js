...
const assertAddLimitedLockWhitelistAddress =
    (btcClient, rskClient) => (address, maxTransferValue) => async () => {
        return assertAddOneOffWhitelistAddress(btcClient, rskClient)(address, maxTransferValue)();
    };

const assertAddOneOffWhitelistAddress =
    (btcClient, rskClient) => (address, maxTransferValue) => async () => {
        const utils = rskUtilsLegacy.with(btcClient, rskClient);

        const initialWhitelistSize = Number(
            await rskClient.rsk.bridge.methods.getLockWhitelistSize().call()
        );
        ...
        const addOneOffLockWhitelistAddressMethod =
            rskClient.rsk.bridge.methods.addOneOffLockWhitelistAddress(address, maxTransferValue);

        await utils.sendTxWithCheck(
            addOneOffLockWhitelistAddressMethod,
            (addResult) => expect(Number(addResult)).to.equal(1),
            WHITELIST_CHANGE_ADDR
        );

        const addResult = Number(
            await rskClient.rsk.bridge.methods
                .addOneOffLockWhitelistAddress(address, maxTransferValue)
                .call({ from: WHITELIST_CHANGE_ADDR })
        );

        expect(addResult).to.equal(-1);

        const finalLockWhitelistSize = Number(
            await rskClient.rsk.bridge.methods.getLockWhitelistSize().call()
        );
...
const assertAddLockWhitelistAddress =
    (btcClient, rskClient) => (address, maxTransferValue) => async () => {
        const utils = rskUtilsLegacy.with(btcClient, rskClient);
        ...
        const addLockWhitelistAddressMethod = rskClient.rsk.bridge.methods.addLockWhitelistAddress(
            address,
            maxTransferValue
        );

        await utils.sendTxWithCheck(
            addLockWhitelistAddressMethod,
            (addResult) => expect(Number(addResult)).to.be.equal(1),
            WHITELIST_CHANGE_ADDR
        );
...
const assertAddUnlimitedWhitelistAddress = (btcClient, rskClient) => (address) => async () => {
    ...
    const addUnlimitedLockWhitelistAddressMethod =
        rskClient.rsk.bridge.methods.addUnlimitedLockWhitelistAddress(address);

    await utils.sendTxWithCheck(
        addUnlimitedLockWhitelistAddressMethod,
        (addResult) => expect(Number(addResult)).to.equal(1),
        WHITELIST_CHANGE_ADDR
    );
    ...
    const addResult = Number(
        await rskClient.rsk.bridge.methods
            .addUnlimitedLockWhitelistAddress(address)
            .call({ from: WHITELIST_CHANGE_ADDR })
    );

    expect(addResult).to.equal(-1);

    const finalWhitelistSize = Number(
        await rskClient.rsk.bridge.methods.getLockWhitelistSize().call()
    );
...
const assertRemoveWhitelistAddress = (btcClient, rskClient) => (address) => async () => {
    ...
    const removeLockWhitelistAddressMethod =
        rskClient.rsk.bridge.methods.removeLockWhitelistAddress(address);

    await utils.sendTxWithCheck(
        removeLockWhitelistAddressMethod,
        (removeResult) => expect(Number(removeResult)).to.equal(1),
        WHITELIST_CHANGE_ADDR
    );

    await assertWhitelistAddressPresence(rskClient)(address, false)();

    const removeResult = Number(
        await rskClient.rsk.bridge.methods
            .removeLockWhitelistAddress(address)
            .call({ from: WHITELIST_CHANGE_ADDR })
    );
...
const assertWhitelistAddressPresence = (rskClient) => (address, present) => async () => {
    ...
    const isPresentFromIndex = async (addressToSearch, size, index) => {
        for (let i = index; i < size; i++) {
            const returnedAddress = await rskClient.rsk.bridge.methods
                .getLockWhitelistAddress(i)
                .call();
...
module.exports = {
    with: (btcClient, rskClient) => {
        return {
            ...
            assertAddUnlimitedWhitelistAddress: assertAddUnlimitedWhitelistAddress(
                btcClient,
                rskClient
            ),
            ...
            assertAddLimitedLockWhitelistAddress: assertAddLimitedLockWhitelistAddress(
                btcClient,
                rskClient
            ),
        };
