...
const assertAddOneOffWhitelistAddress = async (
    ...
    const addOneOffLockWhitelistAddressMethod = bridge.methods.addOneOffLockWhitelistAddress(
        btcAddress,
        maxTransferValueInSatoshis
    );
    ...
    const addResult = Number(
        await bridge.methods
            .addOneOffLockWhitelistAddress(btcAddress, maxTransferValueInSatoshis)
...
const assertAddUnlimitedWhitelistAddress = async (rskTxHelper, btcAddress) => {
    ...
    const addUnlimitedLockWhitelistAddressMethod =
        bridge.methods.addUnlimitedLockWhitelistAddress(btcAddress);
    ...
    const addResult = Number(
        await bridge.methods
            .addUnlimitedLockWhitelistAddress(btcAddress)
            .call({ from: WHITELIST_CHANGE_ADDR })
    );
...
const assertRemoveWhitelistAddress = async (rskTxHelper, btcAddress) => {
    ...
    const removeResult = Number(
        await bridge.methods
            .removeLockWhitelistAddress(btcAddress)
            .call({ from: WHITELIST_CHANGE_ADDR })
    );
