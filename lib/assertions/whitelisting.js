 ...
 */
const assertAddLimitedLockWhitelistAddress = async (
    rskTxHelper,
    btcAddress,
    maxTransferValueInSatoshis
) => {
    return await assertAddOneOffWhitelistAddress(
        rskTxHelper,
        btcAddress,
        maxTransferValueInSatoshis
    );
 ...
 * @param {number} maxTransferValueInSatoshis max transfer value in satoshis that the `btcAddress` is allowed to transfer in a pegin transaction.
 */
const assertAddOneOffWhitelistAddress = async (
    rskTxHelper,
    btcAddress,
    maxTransferValueInSatoshis
) => {
    ...
    const initialWhitelistSize = Number(await bridge.getLockWhitelistSize());

    const unlocked = await rskUtils.getUnlockedAddress(
        rskTxHelper,
        WHITELIST_CHANGE_PK,
    ...
    await rskUtils.sendTxWithCheck(
        ...
        [btcAddress, maxTransferValueInSatoshis],
        WHITELIST_CHANGE_ADDR,
        (addResult) => expect(Number(addResult)).to.equal(1)
    );

    const addResult = Number(
        await bridge.addOneOffLockWhitelistAddress.staticCall(
            btcAddress,
            maxTransferValueInSatoshis,
            { from: WHITELIST_CHANGE_ADDR }
        )
    );

    expect(addResult).to.equal(-1);

    const finalLockWhitelistSize = Number(await bridge.getLockWhitelistSize());
 ...
 * @param {number} maxTransferValueInSatoshis max transfer value in satoshis that the `btcAddress` is allowed to transfer in a pegin transaction.
 */
const assertAddLockWhitelistAddress = async (
    rskTxHelper,
    btcAddress,
    maxTransferValueInSatoshis
) => {
...
const assertAddUnlimitedWhitelistAddress = async (rskTxHelper, btcAddress) => {
    ...
    const addResult = Number(
        await bridge.addUnlimitedLockWhitelistAddress.staticCall(btcAddress, {
            from: WHITELIST_CHANGE_ADDR,
        })
    );
    ...
    const finalWhitelistSize = Number(await bridge.getLockWhitelistSize());
...
const assertRemoveWhitelistAddress = async (rskTxHelper, btcAddress) => {
    ...
    const removeResult = Number(
        await bridge.removeLockWhitelistAddress.staticCall(btcAddress, {
            from: WHITELIST_CHANGE_ADDR,
        })
    );
...
const assertWhitelistAddressPresence = async (rskTxHelper, btcAddress, present) => {
    ...
    const whitelistSize = Number(await bridge.getLockWhitelistSize());

    const isPresentFromIndex = async (addressToSearch, size, index) => {
        for (let i = index; i < size; i++) {
            const returnedAddress = await bridge.getLockWhitelistAddress(i);
