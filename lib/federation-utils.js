const {
  KEY_TYPE_BTC, KEY_TYPE_RSK,
  KEY_TYPE_MST,
  nextFederationCreationBlockNumberStorageIndex
...
const getNextFederationCreationBlockNumber = async (rskTxHelper) => {
  const nextFederationCreationBlockNumberRlpEncoded = await rskTxHelper.getClient().rsk.getStorageBytesAt(BRIDGE_ADDRESS, nextFederationCreationBlockNumberStorageIndex);
  if(nextFederationCreationBlockNumberRlpEncoded === '0x0') {
      return -1;
  }
  const nextFederationCreationBlockNumber = getBridgeStorageValueDecodedToNumber(nextFederationCreationBlockNumberRlpEncoded);
  return nextFederationCreationBlockNumber;
};
