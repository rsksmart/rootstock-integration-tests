...
const getActiveFederationPublicKeys = async (bridge) => {
    ...
    for (let i = 0; i < initialFederationSize; i++) {
        const federatorBtcPublicKey = await bridge.methods
            .getFederatorPublicKeyOfType(i, KEY_TYPE_BTC)
            .call();
        const federatorRskPublicKey = await bridge.methods
            .getFederatorPublicKeyOfType(i, KEY_TYPE_RSK)
            .call();
        const federatorMstPublicKey = await bridge.methods
...
const getProposedFederationPublicKeys = async (bridge) => {
    ...
    for (let i = 0; i < proposedFederationSize; i++) {
        const federatorBtcPublicKey = await bridge.methods
            .getProposedFederatorPublicKeyOfType(i, KEY_TYPE_BTC)
            .call();
        const federatorRskPublicKey = await bridge.methods
            .getProposedFederatorPublicKeyOfType(i, KEY_TYPE_RSK)
            .call();
        const federatorMstPublicKey = await bridge.methods
...
const getRetiringFederationPublicKeys = async (bridge) => {
    ...
    for (let i = 0; i < retiringFederationSize; i++) {
        const federatorBtcPublicKey = await bridge.methods
            .getRetiringFederatorPublicKeyOfType(i, KEY_TYPE_BTC)
            .call();
        const federatorRskPublicKey = await bridge.methods
            .getRetiringFederatorPublicKeyOfType(i, KEY_TYPE_RSK)
            .call();
        const federatorMstPublicKey = await bridge.methods
