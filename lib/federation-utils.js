...
const fundNewFederators = async (rskTxHelper, newFederationConfig) => {
  for(const newFederatorConfig of newFederationConfig) {
      ...
      const initialAddressBalanceInWeis = Number(await rskTxHelper.getBalance(newFederatorRskAddress));
      if(initialAddressBalanceInWeis !== 0) {
        continue; // No need to fund if the address already has funds
      }
