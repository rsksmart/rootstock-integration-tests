...
const { compressPublicKey } = require('./rsk-utils');
...
const FEDERATE_COLORS = [colors.blue, colors.cyan, colors.magenta, colors.red, colors.yellow].map(
    (f) => f.bind(colors)
);
...
const parseActivations = (forks) => {
    let activations = { activationHeights: {} };
...
const startFederate = async (index, fedConfig, blockHashCheckpoint, bookkeepingConfigurations) => {
    ...
    federateConfig.customConfig['federator.signers.BTC.bookkeeping.difficultyTarget'] =
        bookkeepingConfigurations.difficultyTarget;
    federateConfig.customConfig['federator.signers.BTC.bookkeeping.informerInterval'] =
        bookkeepingConfigurations.informerInterval;
    federateConfig.customConfig['federator.signers.BTC.bookkeeping.maxAmountBlockHeaders'] =
        bookkeepingConfigurations.maxAmountBlockHeaders;
    federateConfig.customConfig['federator.signers.BTC.bookkeeping.maxChunkSizeToHsm'] =
        bookkeepingConfigurations.maxChunkSizeToHsm;
    ...
    try {
        ...
        if (fedConfig.type === 'hsm') {
            ...
            const hsm = new TcpSignerRunner(fedConfig.id, hsmPort, [
                `-c${blockHashCheckpoint}`,
            ...
            expect(compressedPublicKeys).to.be.deep.equal(
                Object.values(fedConfig.publicKeys),
                `HSM ${fedConfig.id} public keys do not match the expected ones`
            );
            fedRunner.hsm = hsm;
        }
