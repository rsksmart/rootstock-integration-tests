...
const FEDERATE_OUTPUT = 'federate-{index}:';
...
const startFederate = async (index, fedConfig, blockHashCheckpoint, bookkeepingConfigurations) => {
    ...
    const federateConfig = Object.assign({}, fedConfig, {
        stderr: fedStderr,
    ...
    try {
        ...
        if(Runners.hosts.federates.length > 1) {
            await waitForSync(getRskTransactionHelpers(Runners.hosts.federates));
        }
