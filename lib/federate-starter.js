...
var getFederateOutputPrefix = function(index, label) {
...
const startFederate = async (index, config, latestBlockHash, bookkeepingConfigurations) => {
    
    bookkeepingConfigurations = config.bookkeepingConfigurations || bookkeepingConfigurations;

    const federateOutputPrefix = getFederateOutputPrefix(index, config.federationId);
    ...
    var federateConfig = Object.assign({}, config, {
        stderr: fedStderr,
        logbackFile: config.logbackFile,
    ...
    var fedStdout = new LineWrapper({ prefix: federateOutputPrefix });
    ...
    try {
        ...
        for (keyType of KEY_TYPES) {
            if (config.hsmConfigs != null && config.hsmConfigs[keyType] != null) {
                ...
                var signerId = keyType.toUpperCase();
                ...
                try {
                    ...
                    let hsmGenericStdOut = new LineWrapper({ prefix: getHsmOutputPrefix(index, 'undetermined') });
                    hsmGenericStdOut.pipe(process.stdout);
        ...
        var fedRunner = new FederateRunner(federateConfig);
        fedRunner.hsms = hsms;
