...
const execute = (description, newFederationConfig, segwitToSegwitFederationChange = false) => {
    ...
    describe(description, async function() {
        ...
        before(async () => {
            ...
            const bridgeState = await getBridgeState(getHostUrl(rskTxHelper));
        ...
        it('should create the SVP Fund transaction on the next updateCollections call', async function () {
            ...
            const initialBridgeState = await getBridgeState(getHostUrl(rskTxHelper));
    
            await rskUtils.waitAndUpdateBridge(rskTxHelper);
    
            const finalBridgeState = await getBridgeState(getHostUrl(rskTxHelper));
        ...
        it('should migrate utxos', async () => {
            ...
            const bridgeStateBeforeMigration = await getBridgeState(getHostUrl(rskTxHelper));
            ...
            // Start migration
            await rskUtils.waitAndUpdateBridge(rskTxHelper);
            const bridgeStateAfterUpdatingCollections = await getBridgeState(getHostUrl(rskTxHelper));
            ...
            const bridgeStateAfterMiningPegout = await getBridgeState(getHostUrl(rskTxHelper));
            ...
            const migrationReleaseBtcTransactionInputHashes = allMigrationInputs.map(input => Buffer.from(input.hash).reverse().toString('hex')).sort((a, b) => a.localeCompare(b));
            const oldFederationUtxosHashes = oldFederationUtxos.map(utxo => utxo.btcTxHash).sort((a, b) => a.localeCompare(b));
