...
const { removePrefix0x, ensure0x, splitStringIntoChunks, wait } = require('../utils');
...
const execute = (description, newFederationConfig, makeDonationPegins = false) => {
    ...
    describe(description, async function() {
        ...
        it('should migrate utxos', async () => {
            ...
            expect(bridgeStateBeforeActivation.activeFederationUtxos.length).to.be.equal(migrationReleaseBtcTransaction.ins.length, 'The migration transaction should have the same number of inputs as the active federation utxos before activation.');
