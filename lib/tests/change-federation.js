...
const execute = (description, newFederationConfig, makeDonationPegins = false) => {
    ...
    describe(description, async function() {
        ...
        before(async () => {
            ...
            if(makeDonationPegins) {
                // Making donation pegins to ensure there are enough utxos for migration.
        ...
        it('should migrate utxos', async () => {
            ...
            await rskTxHelper.mine(FUNDS_MIGRATION_AGE_SINCE_ACTIVATION_BEGIN + 1);

            // Start migration
            await rskUtils.waitAndUpdateBridge(rskTxHelper);
            const bridgeStateAfterUpdatingCollections = await getBridgeState(rskTxHelper.getClient());
            
            const migrationPegouts = bridgeStateAfterUpdatingCollections.pegoutsWaitingForConfirmations;
            ...
            const { result: wasPegoutBroadcasted, attempts } = await retryWithCheck(method, pegoutIsBroadcasted => pegoutIsBroadcasted, 20, 2000);
            
            console.log('wasPegoutBroadcasted: ', wasPegoutBroadcasted)
            ...
            const expectedCountOfSignatures = (Math.floor(initialFederationPublicKeys.length / 2) + 1) * migrationPegouts.length;
            await waitForExpectedCountOfAddSignatureEventsToBeEmitted(rskTxHelpers, latestBlockNumber, expectedCountOfSignatures);
            ...
            await rskUtils.findEventInBlock(rskTxHelper, PEGOUT_EVENTS.RELEASE_BTC.name, latestBlockNumber, blockNumberAfterRelease, foundEvent => {
            ...
            if(isLovellActive) {
                expect(allMigrationInputs.length).to.be.equal(bridgeStateBeforeActivation.activeFederationUtxos.length + 1, 'The migration transactions should have one more input than the active federation utxos before activation, due to the utxo that the proposed federation sent to the active federation that should also get migrated.');
            } else {
                expect(allMigrationInputs.length).to.be.equal(bridgeStateBeforeActivation.activeFederationUtxos.length, 'The migration transactions should have the same number of inputs as the active federation utxos before activation.');
            }
            ...
            const oldFederationUtxosHashes = bridgeStateBeforeActivation.activeFederationUtxos.map(utxo => utxo.btcTxHash).sort();

            if(isLovellActive) {
                // Adding the utxo that the proposed federation sent to the active federation that should also get migrated.
                oldFederationUtxosHashes.push(Buffer.from(svpSpendBtcTransaction.outs[0].hash).reverse().toString('hex')).sort();
            }
            ...
            const finalBridgeState = await getBridgeState(rskTxHelper.getClient());
            expect(finalBridgeState.activeFederationUtxos.length).to.be.equal(migrationPegouts.length, 'There should be one utxo in the active federation now.');
