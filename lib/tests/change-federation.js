...
const execute = (description, newFederationConfig, makeDonationPegins = false) => {
    ...
    describe(description, async function() {
        ...
        it('should activate federation', async () => {
            ...
            expect(retiringFederationPublicKeys.sort()).to.be.deep.equal(initialFederationPublicKeys.sort(), 'The retiring federation public keys should be the same as the initial federation public keys.');
        ...
        it('should migrate utxos', async () => {
            ...
            if(isLovellActive) {
                expect(migrationReleaseBtcTransaction.ins.length).to.be.equal(bridgeStateBeforeActivation.activeFederationUtxos.length + 1, 'The migration transaction should have one more input than the active federation utxos before activation, due to the utxo that the proposed federation sent to the active federation that should also get migrated.');
            } else {
                expect(migrationReleaseBtcTransaction.ins.length).to.be.equal(bridgeStateBeforeActivation.activeFederationUtxos.length, 'The migration transaction should have the same number of inputs as the active federation utxos before activation.');
            }

            const migrationReleaseBtcTransactionInputHashes = migrationReleaseBtcTransaction.ins.map(input => input.hash.reverse().toString('hex')).sort();
            ...
            if(isLovellActive) {
                ...
                oldFederationUtxosHashes.push(svpSpendBtcTransaction.outs[0].hash.reverse().toString('hex')).sort();
            }
            ...
            expect(finalBridgeState.activeFederationUtxos.length).to.be.equal(1, 'There should be one utxo in the active federation now.');
