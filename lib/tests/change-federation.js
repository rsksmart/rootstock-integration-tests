...
const execute = (description, newFederationConfig) => {
    ...
    describe(description, async function() {
        ...
        it('should not be able to call `createFederation` without authorization', async () => {
            ...
            // Simulating the transaction to get the error code.

            const message = 'The `createFederation` method should not be callable by an unauthorized address.';

            const result1 = await createFederationMethod.call({ from: notAuthorized1Address });
            expect(result1).to.be.equal(EXPECTED_UNSUCCESSFUL_RESULT, message);

            const result2 = await createFederationMethod.call({ from: notAuthorized2Address });
            expect(result2).to.be.equal(EXPECTED_UNSUCCESSFUL_RESULT, message);

            const result3 = await createFederationMethod.call({ from: notAuthorized3Address });
            expect(result3).to.be.equal(EXPECTED_UNSUCCESSFUL_RESULT, message);

            // Actually sending the transaction to test if it will cause a change.

            // First create federation vote
            await rskUtils.sendTransaction(rskTxHelper, createFederationMethod, notAuthorized1Address);
            
            // Second create federation vote
            await rskUtils.sendTransaction(rskTxHelper, createFederationMethod, notAuthorized2Address);

            // Third and final create federation vote
            await rskUtils.sendTransaction(rskTxHelper, createFederationMethod, notAuthorized3Address);
        ...
        it('should not be able to call `addFederatorPublicKeyMultikey` without authorization', async () => {
            ...
            const result1 = await addFederatorPublicKeyMultikeyMethod.call({ from: notAuthorized1Address });
            expect(result1).to.be.equal(EXPECTED_UNSUCCESSFUL_RESULT, message);

            const result2 = await addFederatorPublicKeyMultikeyMethod.call({ from: notAuthorized2Address });
            expect(result2).to.be.equal(EXPECTED_UNSUCCESSFUL_RESULT, message);

            const result3 = await addFederatorPublicKeyMultikeyMethod.call({ from: notAuthorized3Address });
            expect(result3).to.be.equal(EXPECTED_UNSUCCESSFUL_RESULT, message);

            // Actually sending the transaction to test if it will cause a change.

            // First add federator public key vote
            await rskUtils.sendTransaction(rskTxHelper, addFederatorPublicKeyMultikeyMethod, notAuthorized1Address);

            // Second add federator public key vote
            await rskUtils.sendTransaction(rskTxHelper, addFederatorPublicKeyMultikeyMethod, notAuthorized2Address);

            // Third and final add federator public key vote
            await rskUtils.sendTransaction(rskTxHelper, addFederatorPublicKeyMultikeyMethod, notAuthorized3Address);
        ...
        it('should not be able to call `commitFederation` without authorization', async () => {
            ...
            const result1 = await commitFederationMethod.call({ from: notAuthorized1Address });
            expect(result1).to.be.equal(EXPECTED_UNSUCCESSFUL_RESULT, message);

            const result2 = await commitFederationMethod.call({ from: notAuthorized2Address });
            expect(result2).to.be.equal(EXPECTED_UNSUCCESSFUL_RESULT, message);

            const result3 = await commitFederationMethod.call({ from: notAuthorized3Address });
            expect(result3).to.be.equal(EXPECTED_UNSUCCESSFUL_RESULT, message);

            // Actually sending the transaction to test if it will cause a change.

            // First commit federation vote
            await rskUtils.sendTransaction(rskTxHelper, commitFederationMethod, notAuthorized1Address);

            // Second commit federation vote
            await rskUtils.sendTransaction(rskTxHelper, commitFederationMethod, notAuthorized2Address);

            // Third and final commit federation vote
            const commitFederationTransactionReceipt = await rskUtils.sendTransaction(rskTxHelper, commitFederationMethod, notAuthorized3Address);
        ...
        it('should not be able to call `rollbackFederation` without authorization', async () => {
            ...
            const result1 = await rollbackFederationMethod.call({ from: notAuthorized1Address });
            expect(result1).to.be.equal(EXPECTED_UNSUCCESSFUL_RESULT, message);

            const result2 = await rollbackFederationMethod.call({ from: notAuthorized2Address });
            expect(result2).to.be.equal(EXPECTED_UNSUCCESSFUL_RESULT, message);

            const result3 = await rollbackFederationMethod.call({ from: notAuthorized3Address });
            expect(result3).to.be.equal(EXPECTED_UNSUCCESSFUL_RESULT, message);

            // Actually sending the transaction to test if it will cause a change.

            // First rollback federation vote
            await rskUtils.sendTransaction(rskTxHelper, rollbackFederationMethod, notAuthorized1Address);

            // Second rollback federation vote
            await rskUtils.sendTransaction(rskTxHelper, rollbackFederationMethod, notAuthorized2Address);

            // Third and final rollback federation vote
            const rollbackFederationTransactionReceipt = await rskUtils.sendTransaction(rskTxHelper, rollbackFederationMethod, notAuthorized3Address);
