    ...
    describe(description, async function() {
        ...
        it('should create the SVP Fund transaction on the next updateCollections call', async function () {
            ...
            if(!segwitToSegwitFederationChange) {
                svpFundTransaction.ins.forEach(input => {
                    expect(input.witness.length).to.be.equal(0, 'The SVP fund transaction inputs should not have witness data when REED800 is active and there has been no previous p2sh to p2wsh federation change.');
                });
            }  else if(segwitToSegwitFederationChange) {
                svpFundTransaction.ins.forEach(input => {
                    expect(input.witness.length).to.be.greaterThan(0, 'The SVP fund transaction inputs should have witness data when REED800 is active and there has been a previous p2sh to p2wsh federation change.');
                });
            }
        ...
        it('should register the SVP Spend transaction and finish the SVP process', async function () {
            ...
            svpSpendBtcTransaction.ins.forEach(input => {
                expect(input.witness.length).to.be.greaterThan(0, 'The SVP Spend transaction inputs should have witness data when REED800 is active.');
            });
