...
const execute = (description) => {
    ...
    const getEstimatedFeesForPegOutAmountMethod = (satoshis) => bridge.methods
    ...
    const triggerReleaseAndAssertQueueCleared = async () => {
        ...
        const expectedPegoutRequestCountBeforeRelease = 0;
        expect(countAfterRelease).to.equal(expectedPegoutRequestCountBeforeRelease,
    ...
    describe(description, () => {
        before(async () => {
            ...
            const btcsForPegin = 2;
            await sendPegin(btcsForPegin);
        });
        ...
        const processPegoutsAndAssertEstimatedFees = async (pegoutTxBlockNumber, numberOfPegoutRequestsExpected, estimatedFees) => {
            await assertPegoutRequestsCount(numberOfPegoutRequestsExpected);
        ...
        describe('getEstimatedFeesForNextPegOutEvent', () => {
            it('with no pegout request queued should get a correct fee estimated', async () => {
                // Arrange & Act
                ...
                await processPegoutsAndAssertEstimatedFees(pegoutRequestTx.blockNumber, numberOfPegoutRequestsExpected, estimatedFeesForNextPegOut);
            });

            it('with one pegout request queued should get a correct fee estimated', async () => {
                // Arrange
                const pegoutRequestTx =  await sendReleaseRequest(MINIMUM_PEGOUT_AMOUNT_IN_SATOSHIS);
