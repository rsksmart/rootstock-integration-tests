...
const getReleaseBtcTransactionFeeInSatoshis = (pegoutsEvents) => {
    ...
    expect(pegoutTxCreated, 'pegout_transaction_created for the release BTC tx should exist').to
        .exist;
    ...
    const totalOutputsSum = btcTx.outs.reduce((acc, o) => acc + Number(o.value), 0);
...
const execute = (description) => {
    ...
    const getQueuedPegoutsCount = async () => Number(await bridge.getQueuedPegoutsCount());

    const getEstimatedFeesForNextPegOutEvent = async () =>
        Number(await bridge.getEstimatedFeesForNextPegOutEvent());

    const getEstimatedFeesForPegOutAmountCall = async (satoshis) =>
        Number(await bridge.getEstimatedFeesForPegOutAmount(satoshisToWeis(satoshis)));
    ...
    const triggerReleaseAndAssertQueueCleared = async () => {
        ...
        expect(countAfterRelease).to.equal(
            expectedPegoutRequestCountAfterRelease,
            `Number of pegout requests should be ${expectedPegoutRequestCountAfterRelease} in the queue after triggering release`
        );
    };

    const sendReleaseRequest = async (satoshisAmount) =>
    ...
    const assertEstimatedFeesEqualsActualFees = async (
        pegoutBlockNumber,
    ...
    const assertPegoutRequestsCount = async (expectedPegoutRequestsCount) => {
        ...
        expect(pegoutCount).to.equal(
            expectedPegoutRequestsCount,
            `Expected ${expectedPegoutRequestsCount} pegout requests to be in the queue but got ${pegoutCount}`
        );
    };
    ...
    describe(description, () => {
        ...
        describe('getEstimatedFeesForNextPegOutEvent', () => {
            it('estimates the correct fee with no pegout requests queued', async () => {
                ...
                await triggerReleaseAndAssertEstimatedFeesMatchActualFees(
                    pegoutRequestTx.blockNumber,
                    numberOfPegoutRequestsExpected,
                    estimatedFeesForNextPegOut
                );
            });
        ...
        describe('getEstimatedFeesForPegOutAmount', () => {
            ...
            it('estimates the correct fee with no pegout requests queued', async () => {
                // Arrange & Act
                const estimatedFeesForNextPegOut = await getEstimatedFeesForPegOutAmountCall(
                    PEGOUT_REQUEST_AMOUNT_IN_SATOSHIS
                );
            ...
            it('should revert when pegout amount is below minimum pegout', async () => {
                await assertContractCallFails(bridge, 'getEstimatedFeesForPegOutAmount', [
