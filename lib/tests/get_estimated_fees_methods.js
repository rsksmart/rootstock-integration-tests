...
const { satoshisToBtc, satoshisToWeis } = require('@rsksmart/btc-eth-unit-converter');
...
const { triggerRelease } = require('../rsk-utils');
...
const execute = (description) => {
    ...
    const setupFedUtxosAndAddAPegoutRequestToTheBridge = async () => {
        ...
        expect(countAfterRelease).to.equal(1,
            'One pegout request should have been added to the queue after sending a pegout request to the Bridge contract'
        );
    };
    ...
    const sendPegoutWithEstimatedFees = async (estimatedFeesInSatoshis) => {
        const secondPegoutAmountInSatoshis =
            MINIMUM_PEGOUT_AMOUNT_IN_SATOSHIS + estimatedFeesInSatoshis;
        await sendTxToBridge(
            rskTxHelper,
        ...
        describe('getEstimatedFeesForPegOutAmount', () => {
            it('should build pegouts from getEstimatedFeesForPegOutAmount when one pegout is queued and when none are queued, and process all pegouts', async () => {
                ...
                expect(estimatedFeesForPegout).to.be.greaterThan(
                    0,
                    shouldReturnGreaterThanZeroErrorMessage('getEstimatedFeesForPegOutAmount')
                );
