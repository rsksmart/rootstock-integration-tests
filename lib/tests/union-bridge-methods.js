...
const {
  ...
  UNION_BRIDGE_AUTHORIZER_1_PK, UNION_BRIDGE_AUTHORIZER_2_PK, UNION_BRIDGE_AUTHORIZER_3_PK,
  UNION_BRIDGE_AUTHORIZER_DEPLOYER_PK,
  UNION_RESPONSE_CODES, UNION_BRIDGE_ADDRESS, UNION_BRIDGE_STORAGE_INDICES,
  INITIAL_UNION_LOCKING_CAP, UNION_LOCKING_CAP_INCREMENTS_MULTIPLIER, UNION_BRIDGE_EVENTS
} = require("../constants/union-bridge-constants");

const {
  deployUnionBridgeContract, deployUnionBridgeAuthorizerContract
...
const INITIAL_MAX_LOCKING_CAP_INCREMENT = ethToWeis(Number(weisToEth(INITIAL_UNION_LOCKING_CAP)) * UNION_LOCKING_CAP_INCREMENTS_MULTIPLIER);
const NEW_LOCKING_CAP_1 = ethToWeis(weisToEth(INITIAL_MAX_LOCKING_CAP_INCREMENT) - 0.2);
const NEW_LOCKING_CAP_2 = ethToWeis(weisToEth(INITIAL_MAX_LOCKING_CAP_INCREMENT) - 0.1);
...
const execute = (description) => {
  describe(description, () => {
    before(async () => {
      ...
      bridgeTxParser = new BridgeTransactionParser(rskClient);
    ...
    it('should increaseUnionBridgeLockingCap fail when calling from an unauthorized address', async () => {
      ...
      // Act & Assert
      await assertContractCallFails(bridge.methods.increaseUnionBridgeLockingCap(NEW_LOCKING_CAP_1), {
    ...
    it('should increaseUnionBridgeLockingCap be successful when caller is authorized', async () => {
      ...
      await assertLogUnionLockingCapIncreased(txReceipt.transactionHash, unionBridgeAuthorizerContractAddress, INITIAL_UNION_LOCKING_CAP, NEW_LOCKING_CAP_1);
    });
    ...
    it("should increaseUnionBridgeLockingCap be successful when vote again for another value", async () => {
      ...
      await assertLogUnionLockingCapIncreased(txReceipt.transactionHash, unionBridgeAuthorizerContractAddress, NEW_LOCKING_CAP_1, NEW_LOCKING_CAP_2);
    });

    it("should increaseUnionBridgeLockingCap remains unchanged when trying to decrease the locking cap", async () => {
      // Arrange
      const lockingCapBeforeUpdate = await getUnionBridgeLockingCap();
    ...
    it("should requestUnionRbtc be successful when caller is the union contract address", async () => {
      // Arrange
    ...
    it("should setUnionBridgeTransferPermissions return UNAUTHORIZED_CALLER when caller is unauthorized", async () => {
      ...
      // Act & Assert
      await assertContractCallFails(bridge.methods.setUnionBridgeTransferPermissions(REQUEST_PERMISSION_DISABLED, RELEASE_PERMISSION_DISABLED), {
        from: unauthorizedAddress
      });
    });
    ...
    it("should setUnionBridgeTransferPermissions vote be successful when caller is authorized", async () => {
      ...
      await assertLogUnionTransferPermissionsSet(txReceipt.transactionHash, unionBridgeAuthorizerContractAddress, RELEASE_PERMISSION_DISABLED, RELEASE_PERMISSION_DISABLED);
    });
    ...
    it("should increaseUnionBridgeLockingCap vote be successful when transfer permissions are disabled", async () => {
      ...
      await assertLogUnionLockingCapIncreased(txReceipt.transactionHash, unionBridgeAuthorizerContractAddress, unionLockingCapBeforeUpdate, newLockingCap.toString());
    });
    ...
    it("should setUnionTransferPermissions vote be successful when voting to enable only request permission", async () => {
      ...
      await assertLogUnionTransferPermissionsSet(txReceipt.transactionHash, unionBridgeAuthorizerContractAddress, REQUEST_PERMISSION_ENABLED, RELEASE_PERMISSION_DISABLED);
    });
    ...
    it("should setUnionBridgeTransferPermissions vote be successful when voting to enable only release permission", async () => {
      ...
      await assertLogUnionTransferPermissionsSet(txReceipt.transactionHash, unionBridgeAuthorizerContractAddress, REQUEST_PERMISSION_DISABLED, RELEASE_PERMISSION_ENABLED);
    });
    ...
    it("should setTransferPermissions vote be successful when voting to enable both request and release permissions", async () => {
      ...
      // Assert transfer permissions remains unchanged
      await assertUnionTransferredPermissions(REQUEST_PERMISSION_ENABLED, RELEASE_PERMISSION_ENABLED);
      await assertLogUnionTransferPermissionsSet(txReceipt.transactionHash, unionBridgeAuthorizerContractAddress, REQUEST_PERMISSION_ENABLED, RELEASE_PERMISSION_ENABLED);
    });
    ...
    it("should releaseUnionBridgeRbtc fail when surpass weisTransferredBalance", async () => {
      ...
      await assertLogUnionTransferPermissionsSet(txReceipt.transactionHash, BRIDGE_ADDRESS, REQUEST_PERMISSION_DISABLED, RELEASE_PERMISSION_DISABLED);
...
const createAndFundAccounts = async () => {
  ...
  await rskUtils.sendFromCow(rskTxHelper, unionBridgeAuthorizerMember1Address, btcToWeis(0.1));
  await rskUtils.sendFromCow(rskTxHelper, unionBridgeAuthorizerMember2Address, btcToWeis(0.1));
  await rskUtils.sendFromCow(rskTxHelper, unionBridgeAuthorizerMember3Address, btcToWeis(0.1));
  await rskUtils.sendFromCow(rskTxHelper, unionBridgeAuthorizerAddress, btcToWeis(0.1));
}
...
const deployAndInitUnionAuthorizerContract = async () => {
  ...
  const multisigInitMethod = unionBridgeAuthorizerContract.methods.init(multisigMembers, UNION_AUTHORIZER_VOTING_PERIOD_IN_BLOCKS);
...
const assertNoAddressIsStored = async () => {
  const unionBridgeAddressEncoded = await rskClient.rsk.getStorageBytesAt(BRIDGE_ADDRESS, UNION_BRIDGE_STORAGE_INDICES.UNION_BRIDGE_CONTRACT_ADDRESS);
...
const increaseUnionBridgeLockingCap = async (newLockingCap) => {
  ...
  const txReceiptSecondVote = await voteToIncreaseUnionBridgeLockingCap(newLockingCap, unionBridgeAuthorizerMember2Address);
  assertIncreaseUnionLockingCapVotedEventWasEmitted(newLockingCap, unionBridgeAuthorizerMember2Address, txReceiptSecondVote);
...
const assertIncreaseUnionLockingCapVotedEventWasEmitted = (newLockingCap, authorizedMember, txReceipt) => {
  ...
  expect(foundIncreaseLockingCapEvent, `Expected to find event with name "${IncreaseUnionLockingCapVotedEvent}"`).to.not.be.undefined;
...
const assertStoredUnionLockingCap = async (expectedLockingCap) => {
  const unionLockingCapEncoded = await rskClient.rsk.getStorageBytesAt(BRIDGE_ADDRESS, UNION_BRIDGE_STORAGE_INDICES.UNION_BRIDGE_LOCKING_CAP);
...
const assertLogUnionLockingCapIncreased = async (txHash, callerAddress, previousLockingCap, newLockingCap) => {
  const txReceipt = await rskClient.eth.getTransactionReceipt(txHash);
  const unionEvents = decodeUnionLogs(txReceipt);
...
const assertLogUnionRbtcReleased = async (txReceipt, callerAddress, amountReleased) => {
  ...
  const eventArguments = foundEvent.returnValues;
  expect(eventArguments.receiver.toLowerCase()).to.equal(callerAddress.toLowerCase());
  expect(eventArguments.amount).to.equal(amountReleased);
}

const assertNoWeisTransferredToUnionBridgeIsStored = async () => {
  const weisTransferredEncoded = await rskClient.rsk.getStorageBytesAt(BRIDGE_ADDRESS, UNION_BRIDGE_STORAGE_INDICES.WEIS_TRANSFERRED_TO_UNION_BRIDGE);
...
const assertNoUnionTransferredPermissionsIsStored = async () => {
  const actualRequestPermissionEncoded = await rskClient.rsk.getStorageBytesAt(BRIDGE_ADDRESS, UNION_BRIDGE_STORAGE_INDICES.UNION_BRIDGE_REQUEST_ENABLED);
  const actualReleasePermissionEncoded = await rskClient.rsk.getStorageBytesAt(BRIDGE_ADDRESS, UNION_BRIDGE_STORAGE_INDICES.UNION_BRIDGE_RELEASE_ENABLED);
...
const setUnionTransferPermissions = async (requestEnabled, releaseEnabled) => {
  ...
  // Second vote from member 2 (reaches the threshold and executes)
  const txReceiptSecondVote = await voteToSetUnionTransferPermissions(requestEnabled, releaseEnabled, unionBridgeAuthorizerMember2Address);
  assertUnionTransferPermissionsVotedEventWasEmitted(requestEnabled, releaseEnabled, unionBridgeAuthorizerMember2Address, txReceiptSecondVote);
...
const voteToSetUnionTransferPermissions = async (requestEnabled, releaseEnabled, authorizedMember) => {
  const setUnionTransferPermissionsVote = unionBridgeAuthorizerContract.methods.voteToSetUnionTransferPermissions(requestEnabled, releaseEnabled);
  return await rskUtils.sendTransaction(rskTxHelper, setUnionTransferPermissionsVote, authorizedMember, 0, 300000);
};

const assertUnionTransferPermissionsVotedEventWasEmitted = (requestEnabled, releaseEnabled, authorizedMember, txReceipt) => {
...
const decodeUnionLogs = (txReceipt) => {
  const events = [];
  for (let log of txReceipt.logs) {
    ...
    const abiElement = findAbiElementBySignature(eventSignature);
    if (!abiElement) {
      continue;
    }
    const event = decodeUnionLog(log, abiElement);
    events.push(event);
  }
  return events;
}

const findAbiElementBySignature = (eventSignature) => {
  const unionAndBridgeAbis = [
    unionBridgeAuthorizerContract.options.jsonInterface,
    bridge.options.jsonInterface
  ];

  for (let abi of unionAndBridgeAbis) {
...
const decodeUnionLog = (log, abiElement) => {
