...
const {CHANGE_LOCKING_CAP_AUTHORIZER_PK, CHANGE_UNION_BRIDGE_CONTRACT_ADDRESS_AUTHORIZER_PK,
  CHANGE_TRANSFER_PERMISSIONS_AUTHORIZER_PK, UNION_RESPONSE_CODES, UNION_BRIDGE_ADDRESS, UNION_BRIDGE_STORAGE_INDICES,
...
const execute = (description) => {
  describe(description, () => {
    ...
    it('should increaseUnionBridgeLockingCap be successful when caller address is authorized', async () => {
      // Act
    ...
    it("should increaseUnionBridgeLockingCap be successful when authorizer vote again for another value", async () => {
      // Arrange
      ...
      await assertLogUnionLockingCapIncreased(txReceipt, changeLockingCapAuthorizerAddress, NEW_LOCKING_CAP_1, NEW_LOCKING_CAP_2);
    });
...
const createAndFundAccounts = async () => {
  ...
  const unionAuthorizedAddresses = await importAccounts([
    ...
    CHANGE_LOCKING_CAP_AUTHORIZER_PK,
    CHANGE_TRANSFER_PERMISSIONS_AUTHORIZER_PK
  ]);
  ...
  changeTransferPermissionsAuthorizerAddress = unionAuthorizedAddresses[2];
...
const assertLogUnionLockingCapIncreased = async (txReceipt, callerAddress, previousLockingCap, newLockingCap) => {
  ...
  const arguments = foundEvent.returnValues;
  expect(arguments.caller.toLowerCase()).to.equal(callerAddress.toLowerCase());
  expect(arguments.previousLockingCap).to.equal(previousLockingCap);
  expect(arguments.newLockingCap).to.equal(newLockingCap);
};
...
const assertLogUnionTransferPermissionsSet = async (txReceipt, callerAddress, requestEnabled, releaseEnabled) => {
  ...
  const arguments = foundEvent.returnValues;
  expect(arguments.caller.toLowerCase()).to.equal(callerAddress.toLowerCase());
  expect(arguments.enablePowPegToUnionBridge).to.equal(requestEnabled);
  expect(arguments.enableUnionBridgeToPowPeg).to.equal(releaseEnabled);
};
...
const assertLogUnionRbtcRequested = async (txReceipt, callerAddress, amountRequested) => {
  ...
  const arguments = foundEvent.returnValues;
  expect(arguments.requester.toLowerCase()).to.equal(callerAddress.toLowerCase());
  expect(arguments.amount).to.equal(amountRequested);
}
...
const assertLogUnionRbtcReleased = async (txReceipt, callerAddress, amountReleased) => {
  ...
  const arguments = foundEvent.returnValues;
  expect(arguments.receiver.toLowerCase()).to.equal(callerAddress.toLowerCase());
  expect(arguments.amount).to.equal(amountReleased);
}
