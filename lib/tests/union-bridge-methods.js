...
const {CHANGE_LOCKING_CAP_AUTHORIZERS_PKS, CHANGE_UNION_BRIDGE_CONTRACT_ADDRESS_AUTHORIZER_PK,
  CHANGE_TRANSFER_PERMISSIONS_AUTHORIZERS_PKS, UNION_RESPONSE_CODES, UNION_BRIDGE_ADDRESS, UNION_BRIDGE_STORAGE_INDICES,
  INITIAL_LOCKING_CAP, LOCKING_CAP_INCREMENTS_MULTIPLIER, UNION_BRIDGE_EVENTS
...
const execute = (description) => {
  describe(description, () => {
    ...
    it('should increaseUnionBridgeLockingCap first vote be successful when caller address is authorized', async () => {
      // Act
      const txReceipt = await increaseUnionBridgeLockingCap(NEW_LOCKING_CAP_1, changeLockingCapAuthorizer1Address, assertSuccessfulResponseCode);
      // Assert
      const unionLockingCapAfter = await getUnionBridgeLockingCap();
      expect(unionLockingCapAfter).to.equal(INITIAL_LOCKING_CAP);
      await assertNoUnionLockingCapIsStored();
      await assertNoEventWasEmitted(txReceipt);
    });
    ...
    it('should increaseUnionBridgeLockingCap second vote, to a different value than first vote, be successful when caller address is authorizer2', async () => {
      // Act
      const txReceipt = await increaseUnionBridgeLockingCap(NEW_LOCKING_CAP_2, changeLockingCapAuthorizer2Address, assertSuccessfulResponseCode);
      // Assert
      const unionLockingCapAfter = await getUnionBridgeLockingCap();
      expect(unionLockingCapAfter).to.equal(INITIAL_LOCKING_CAP);
      await assertNoUnionLockingCapIsStored();
    ...
    it('should increaseUnionBridgeLockingCap third vote for same first vote value be successful when authorizer_2 vote again', async () => {
      // Act
      const txReceipt = await increaseUnionBridgeLockingCap(NEW_LOCKING_CAP_1, changeLockingCapAuthorizer2Address, assertSuccessfulResponseCode);
      // Assert
    ...
    it("should increaseUnionBridgeLockingCap return INVALID_VALUE when trying to decrease the locking cap", async () => {
      ...
      // Act
      const txReceipt = await increaseUnionBridgeLockingCap(smallerLockingCap.toString(), changeLockingCapAuthorizer2Address, (actualUnionResponseCode) => {
    ...
    it("should setUnionBridgeTransferPermissions first vote be successful when called from authorized caller", async () => {
      const txReceipt = await setUnionTransferPermissions(REQUEST_PERMISSION_DISABLED, RELEASE_PERMISSION_DISABLED, changeTransferPermissionsAuthorizer1Address, assertSuccessfulResponseCode);
    ...
    it("should setUnionBridgeTransferPermissions second vote for different value be successful when called from authorized caller", async () => {
      const txReceipt = await setUnionTransferPermissions(REQUEST_PERMISSION_DISABLED, REQUEST_PERMISSION_ENABLED, changeTransferPermissionsAuthorizer2Address, assertSuccessfulResponseCode);
      // Assert transferred permissions remain unchanged
      await assertNoUnionTransferredPermissionsIsStored();
      await assertNoEventWasEmitted(txReceipt);
    });

    it("should setUnionBridgeTransferPermissions third vote for same value as first vote be successful when called from authorized caller", async () => {
      // Arrange
      const txReceipt = await setUnionTransferPermissions(REQUEST_PERMISSION_DISABLED, RELEASE_PERMISSION_DISABLED, changeTransferPermissionsAuthorizer2Address, assertSuccessfulResponseCode);
      // Assert transferred permissions remain unchanged
      await assertUnionTransferredPermissions(RELEASE_PERMISSION_DISABLED, RELEASE_PERMISSION_DISABLED);
      await assertLogUnionTransferPermissionsSet(txReceipt, changeTransferPermissionsAuthorizer2Address, RELEASE_PERMISSION_DISABLED, RELEASE_PERMISSION_DISABLED);
    });
    ...
    it("should increaseUnionBridgeLockingCap first vote be successful when transfer permissions are disabled", async () => {
      // Arrange
      const unionLockingCapBeforeUpdate = await getUnionBridgeLockingCap();
      const newLockingCap = new BN(unionLockingCapBeforeUpdate).mul(new BN(LOCKING_CAP_INCREMENTS_MULTIPLIER));
      // Act
      const txReceipt = await increaseUnionBridgeLockingCap(newLockingCap.toString(), changeLockingCapAuthorizer1Address, assertSuccessfulResponseCode);
      // Assert that the locking cap remains unchanged
      await assertLockingCap(unionLockingCapBeforeUpdate);
      await assertNoEventWasEmitted(txReceipt);
    });

    it("should increaseUnionBridgeLockingCap second vote be successful when transfer permissions are disabled", async () => {
      // Arrange
      const unionLockingCapBeforeUpdate = await getUnionBridgeLockingCap();
      const newLockingCap = new BN(unionLockingCapBeforeUpdate).mul(new BN(LOCKING_CAP_INCREMENTS_MULTIPLIER));
      // Act
      const txReceipt = await increaseUnionBridgeLockingCap(newLockingCap.toString(), changeLockingCapAuthorizer2Address, assertSuccessfulResponseCode);
      // Assert
      await assertLockingCap(newLockingCap.toString());
      await assertLogUnionLockingCapIncreased(txReceipt, changeLockingCapAuthorizer2Address, unionLockingCapBeforeUpdate, newLockingCap.toString());
    });
    ...
    it("should setUnionTransferPermissions first vote be successful when voting to enable only request permission", async () => {
      // Act
      const txReceipt = await setUnionTransferPermissions(REQUEST_PERMISSION_ENABLED, RELEASE_PERMISSION_DISABLED, changeTransferPermissionsAuthorizer1Address, assertSuccessfulResponseCode);
      // Assert transfer permissions remains disabled
      await assertUnionTransferredPermissions(REQUEST_PERMISSION_DISABLED, RELEASE_PERMISSION_DISABLED);
      await assertNoEventWasEmitted(txReceipt);
    });

    it("should setUnionTransferPermissions second vote be successful when voting to enable only request permission", async () => {
      // Act
      const txReceipt = await setUnionTransferPermissions(REQUEST_PERMISSION_ENABLED, RELEASE_PERMISSION_DISABLED, changeTransferPermissionsAuthorizer2Address, assertSuccessfulResponseCode);
      // Assert
      await assertUnionTransferredPermissions(REQUEST_PERMISSION_ENABLED, RELEASE_PERMISSION_DISABLED);
      await assertLogUnionTransferPermissionsSet(txReceipt, changeTransferPermissionsAuthorizer2Address, REQUEST_PERMISSION_ENABLED, RELEASE_PERMISSION_DISABLED);
    });
    ...
    it("should setUnionBridgeTransferPermissions first vote be successful when voting to enable only release permission", async () => {
      const txReceipt = await setUnionTransferPermissions(REQUEST_PERMISSION_DISABLED, RELEASE_PERMISSION_ENABLED, changeTransferPermissionsAuthorizer1Address, assertSuccessfulResponseCode);
      // Assert transfer permissions remains unchanged
      await assertUnionTransferredPermissions(REQUEST_PERMISSION_ENABLED, RELEASE_PERMISSION_DISABLED);
      await assertNoEventWasEmitted(txReceipt);
    });

    it("should setUnionBridgeTransferPermissions second vote be successful when voting to enable only release permission", async () => {
      const txReceipt = await setUnionTransferPermissions(REQUEST_PERMISSION_DISABLED, RELEASE_PERMISSION_ENABLED, changeTransferPermissionsAuthorizer2Address, assertSuccessfulResponseCode);
      // Assert
      await assertUnionTransferredPermissions(REQUEST_PERMISSION_DISABLED, RELEASE_PERMISSION_ENABLED);
      await assertLogUnionTransferPermissionsSet(txReceipt, changeTransferPermissionsAuthorizer2Address, REQUEST_PERMISSION_DISABLED, RELEASE_PERMISSION_ENABLED);
    });
    ...
    it("should setTransferPermissions first vote be successful when voting to enable both request and release permissions", async () => {
      const txReceipt = await setUnionTransferPermissions(REQUEST_PERMISSION_ENABLED, RELEASE_PERMISSION_ENABLED, changeTransferPermissionsAuthorizer1Address, assertSuccessfulResponseCode);
      // Assert transfer permissions remains unchanged
      await assertUnionTransferredPermissions(REQUEST_PERMISSION_DISABLED, RELEASE_PERMISSION_ENABLED);
      await assertNoEventWasEmitted(txReceipt);
    });

    it("should setTransferPermissions second vote be successful when voting to enable both request and release permissions", async () => {
      const txReceipt = await setUnionTransferPermissions(REQUEST_PERMISSION_ENABLED, RELEASE_PERMISSION_ENABLED, changeTransferPermissionsAuthorizer2Address, assertSuccessfulResponseCode);
      // Assert
      await assertUnionTransferredPermissions(REQUEST_PERMISSION_ENABLED, RELEASE_PERMISSION_ENABLED);
      await assertLogUnionTransferPermissionsSet(txReceipt, changeTransferPermissionsAuthorizer2Address, REQUEST_PERMISSION_ENABLED, RELEASE_PERMISSION_ENABLED);
    });
    ...
    it("should setUnionTransferPermissions first vote be successful when voting to enable both permissions", async () => {
      // Act
      const txReceipt = await setUnionTransferPermissions(REQUEST_PERMISSION_ENABLED, RELEASE_PERMISSION_ENABLED, changeTransferPermissionsAuthorizer1Address, assertSuccessfulResponseCode);
      // Assert transfer permissions remains unchanged
      await assertUnionTransferredPermissions(REQUEST_PERMISSION_DISABLED, RELEASE_PERMISSION_DISABLED);
    ...
    it("should setUnionTransferPermissions second vote be successful when voting to enable both permissions", async () => {
      const txReceipt = await setUnionTransferPermissions(REQUEST_PERMISSION_ENABLED, RELEASE_PERMISSION_ENABLED, changeTransferPermissionsAuthorizer2Address, assertSuccessfulResponseCode);
      // Assert
      await assertUnionTransferredPermissions(REQUEST_PERMISSION_ENABLED, RELEASE_PERMISSION_ENABLED);
      await assertLogUnionTransferPermissionsSet(txReceipt, changeTransferPermissionsAuthorizer2Address, REQUEST_PERMISSION_ENABLED, RELEASE_PERMISSION_ENABLED);
