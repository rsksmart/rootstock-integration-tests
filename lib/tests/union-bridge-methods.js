...
const execute = (description) => {
    describe(description, () => {
        ...
        it('should setUnionBridgeContractAddressForTestnet change union address', async () => {
            // Arrange
            const unionBridgeAddressBeforeUpdate =
                await getUnionBridgeContractAddress(bridgeMethods);
            ...
            // Assert
            const currentUnionBridgeContractAddress =
                await getUnionBridgeContractAddress(bridgeMethods);
        ...
        describe('Super and base events', () => {
            ...
            beforeEach(async () => {
                ...
                await rskUtils.sendTransaction(
                    rskTxHelper,
                    unionBridgeContract.methods.clearSuperEvent(),
                    unionBridgeContractOwnerAddress
                );
                
                await rskUtils.sendTransaction(
                    rskTxHelper,
                    unionBridgeContract.methods.clearBaseEvent(),
                    unionBridgeContractOwnerAddress
                );
            ...
            it('should return empty super and base event bytes after clearing', async () => {
                const superEvent = await bridgeMethods.getSuperEvent().call();
                const baseEvent = await bridgeMethods.getBaseEvent().call();
            ...
            it('should persist super event data when set by the union bridge contract', async () => {
                ...
                await rskUtils.sendTransaction(
                    rskTxHelper,
                    unionBridgeContract.methods.setSuperEvent(payload),
                    unionBridgeContractOwnerAddress
                );

                const stored = await bridgeMethods.getSuperEvent().call();
                expect(stored.toLowerCase()).to.equal(payload.toLowerCase());
                const baseUntouched = await bridgeMethods.getBaseEvent().call();
            ...
            it('should clear super event when clearSuperEvent is called from the union bridge contract', async () => {
                ...
                expect((await bridgeMethods.getSuperEvent().call()).toLowerCase()).to.equal(
                    payload.toLowerCase()
                );
                ...
                expect(await bridgeMethods.getSuperEvent().call()).to.be.null;
            });
            ...
            it('should persist base event data when set by the union bridge contract', async () => {
                ...
                await rskUtils.sendTransaction(
                    rskTxHelper,
                    unionBridgeContract.methods.setBaseEvent(payload),
                    unionBridgeContractOwnerAddress
                );

                const stored = await bridgeMethods.getBaseEvent().call();
                expect(stored.toLowerCase()).to.equal(payload.toLowerCase());
                const superUntouched = await bridgeMethods.getSuperEvent().call();
            ...
            it('should clear base event when clearBaseEvent is called from the union bridge contract', async () => {
                ...
                expect((await bridgeMethods.getBaseEvent().call()).toLowerCase()).to.equal(
                    payload.toLowerCase()
                );
                ...
                expect(await bridgeMethods.getBaseEvent().call()).to.be.null;
            });
            ...
            it('should reject setSuperEvent from a direct bridge call when the caller is not the union bridge', async () => {
                ...
                const method = bridgeMethods.setSuperEvent(payload);
                await method.call({ from: unauthorizedAddress });
            ...
            it('should reject setBaseEvent from a direct bridge call when the caller is not the union bridge', async () => {
                ...
                const method = bridgeMethods.setBaseEvent(payload);
                await method.call({ from: unauthorizedAddress });
            ...
            it('should reject clearSuperEvent from a direct bridge call when the caller is not the union bridge', async () => {
                ...
                const method = bridgeMethods.clearSuperEvent();
                await method.call({ from: unauthorizedAddress });
            ...
            it('should reject clearBaseEvent from a direct bridge call when the caller is not the union bridge', async () => {
                ...
                const method = bridgeMethods.clearBaseEvent();
                await method.call({ from: unauthorizedAddress });
            ...
            it('should reject setSuperEvent when payload length is above the maximum', async () => {
                const method = unionBridgeContract.methods.setSuperEvent(tooLongPayloadHex());
                await rskUtils.assertContractCallFails(method, {
                    from: unionBridgeContractOwnerAddress,
                });
            });

            it('should reject setBaseEvent when payload length is above the maximum', async () => {
                const method = unionBridgeContract.methods.setBaseEvent(tooLongPayloadHex());
                await rskUtils.assertContractCallFails(method, {
                    from: unionBridgeContractOwnerAddress,
                });
            });
        ...
        it('should increaseUnionBridgeLockingCap return UNAUTHORIZED_CALLER when caller is unauthorized', async () => {
            ...
            await rskUtils.assertContractCallFails(
                bridgeMethods.increaseUnionBridgeLockingCap(NEW_LOCKING_CAP_1),
                {
        ...
        it('should setUnionBridgeTransferPermissions return UNAUTHORIZED_CALLER when caller is unauthorized', async () => {
            ...
            // Act & Assert
            await rskUtils.assertContractCallFails(
                bridgeMethods.setUnionBridgeTransferPermissions(
...
const setUnionBridgeContractAddressForTestnet = async (
    ...
    const updateUnionAddressMethod =
        bridgeMethods.setUnionBridgeContractAddressForTestnet(newUnionAddress);
    return rskUtils.sendTxWithCheck(
        rskTxHelper,
        updateUnionAddressMethod,
        fromAddress,
        checkCallback
    );
};
...
const voteToSetUnionTransferPermissions = async (
    ...
    const setUnionTransferPermissionsVote =
        unionBridgeAuthorizerContract.methods.voteToSetUnionTransferPermissions(
            requestEnabled,
            releaseEnabled
        );
    return await rskUtils.sendTransaction(
        rskTxHelper,
        setUnionTransferPermissionsVote,
        authorizedMember,
        0,
        300000
    );
