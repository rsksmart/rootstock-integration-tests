...
const execute = (description) => {
  describe(description, () => {
    ...
    it("should setUnionBridgeContractAddressForTestnet change union address", async () => {
      ...
      // Act
      const txReceipt = await updateUnionAddress(
...
const releaseUnionBridgeRbtcFromUnauthorizedCaller = async (amountToRelease, checkCallback) => {
  ...
  return rskUtils.sendTransaction(rskTxHelper, method, unionBridgeContractCreatorAddress, amountToRelease);
};
...
const buildEventSignatureMap = () => {
  const unionAndBridgeAbis = [unionBridgeAuthorizerContract.options.jsonInterface, bridge.options.jsonInterface];
