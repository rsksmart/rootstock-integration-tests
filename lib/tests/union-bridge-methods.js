...
const execute = (description) => {
  describe(description, () => {
    ...
    it("should increaseUnionBridgeLockingCap vote be successful when transfer permissions are disabled", async () => {
      ...
      // Assert
      await assertLockingCap(rskClient, bridgeMethods, newLockingCap.toString());
...
const voteToIncreaseUnionBridgeLockingCapExpectingRevert = async (newLockingCap) => {
  ...
  await rskUtils.assertContractCallFails(unionBridgeAuthorizerContract.methods.voteToIncreaseUnionLockingCap(newLockingCap), {
...
const findEventInTx = async (txHash, eventName) => {
  const rskClient = rskTxHelper.getClient();
  const txReceipt = await rskTxHelper.getTxReceipt(txHash);
  const unionEvents = rskUtils.decodeLogs(rskClient, txReceipt, unionAndBridgeAbis);
  const expectedEventName = eventName;
  return unionEvents.find((event) => event.name === expectedEventName);
};
...
const assertLogUnionRbtcRequested = async (txHash, amountRequested) => {
  ...
  const foundEvent = await findEventInTx(txHash, expectedEventName);
