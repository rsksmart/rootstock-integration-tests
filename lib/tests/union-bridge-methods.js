...
const execute = (description) => {
  ...
  describe(description, function () {
    ...
    it('should release union bridge RBTC when authorized', async () => {
      ...
      // Assert
      const expectedBalance = substractionInWeis(balanceBeforeRelease, AMOUNT_TO_RELEASE);
    ...
    it('requestUnionRbtc should work when request permission is enabled', async () => {
      ...
      // Assert
      await assertUnionRbtcRequestedEventWasEmitted(txReceipt.blockNumber, newUnionBridgeContractAddress, AMOUNT_TO_REQUEST);
    ...
    it('should return INVALID_VALUE when requesting more than locking cap', async () => {
      ...
      const amountToRequestSurpassingCurrentLockingCap = additionInWeis(currentLockingCap, ethToWeis(1));

      // Act
      const txReceipt = await requestUnionRbtc(amountToRequestSurpassingCurrentLockingCap.toString(), (actualUnionResponseCode) => {
        expect(actualUnionResponseCode).to.equal(UNION_RESPONSE_CODES.INVALID_VALUE);
      });
      ...
      // Assert that the weis transferred balance and union bridge balance remain unchanged
      await assertWeisTransferredToUnionBridge(balanceBeforeRequest);
      await assertUnionBridgeBalance(balanceBeforeRequest);
    });
    ...
    it('should return INVALID_VALUE and disable permissions when releasing more than weis transferred balance', async () => {
      ...
      const amountToReleaseSurpassingCurrentWeisTransferredBalance = additionInWeis(currentWeisTransferredBalance, ethToWeis(1));

      // Act
      const txReceipt = await releaseUnionRbtc(amountToReleaseSurpassingCurrentWeisTransferredBalance, (actualUnionResponseCode) => {
        expect(actualUnionResponseCode).to.equal(UNION_RESPONSE_CODES.INVALID_VALUE);
      });

      // Assert
      await assertTransferPermissions(false, false);
      await assertUnionTransferPermissionsUpdatedEventWasEmitted(txReceipt.blockNumber, BRIDGE_ADDRESS, false, false);
    ...
    it('should vote successfully when first vote to enable back both permissions', async () => {
      // Act
      await setUnionTransferPermissions(true, true, changeTransferPermissionsAuthorizer1Address, (actualResponseCode) => {
        expect(actualResponseCode).to.equal(UNION_RESPONSE_CODES.SUCCESS);
      });
    ...
    it('should request union bridge RBTC when permissions enabled after force pause', async () => {
      ...
      // Act
      const txReceipt = await requestUnionRbtc(AMOUNT_TO_REQUEST, (actualResponseCode) => {
        expect(actualResponseCode).to.equal(UNION_RESPONSE_CODES.SUCCESS);
      });
      ...
      await assertUnionRbtcRequestedEventWasEmitted(txReceipt.blockNumber, newUnionBridgeContractAddress, AMOUNT_TO_REQUEST);
      const expectedWeisTransferredBalance = additionInWeis(weisTransferredBalanceBeforeRequest, AMOUNT_TO_REQUEST);
      const expectedUnionBridgeBalance = additionInWeis(unionBridgeBalanceBeforeRequest, AMOUNT_TO_REQUEST);
      await assertWeisTransferredToUnionBridge(expectedWeisTransferredBalance);
    ...
    it('should release union bridge RBTC when permissions enabled after force pause', async () => {
      ...
      // Assert
      await assertUnionRbtcRequestedEventWasEmitted(txReceipt.blockNumber, newUnionBridgeContractAddress, AMOUNT_TO_RELEASE);
      const expectedWeisTransferredBalance = substractionInWeis(weisTransferredBalanceBeforeRelease, AMOUNT_TO_RELEASE);
      const expectedUnionBridgeBalance = substractionInWeis(unionBridgeBalanceBeforeRelease, AMOUNT_TO_RELEASE);
      await assertWeisTransferredToUnionBridge(expectedWeisTransferredBalance);
      await assertUnionBridgeBalance(expectedUnionBridgeBalance);
    });
    ...
    const substractionInWeis = async (amountInWeis, secondAmountInWeis) => {
      return ethToWeis(Number(weisToEth(amountInWeis)) - Number(weisToEth(secondAmountInWeis)));
    }
    ...
    const assertUnionRbtcRequestedEventWasEmitted = async (blockBeforeRequest, expectedRequester, expectedAmount) => {
      const unionRbtcRequestedEvent = await rskUtils.findEventInBlock(rskTxHelper, UNION_BRIDGE_EVENTS.UNION_RBTC_REQUESTED.name, blockBeforeRequest);
