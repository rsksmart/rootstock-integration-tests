...
const {deployUnionBridgeContract, deployUnionBridgeAuthorizerContract
...
const assertLogUnionLockingCapIncreased = async (txHash, callerAddress, previousLockingCap, newLockingCap) => {
  const txReceipt = await rskTxHelper.getClient().eth.getTransactionReceipt(txHash);
  ...
  for (let abi of unionAndBridgeAbis) {
    for (let element of abi) {
      if (element.type === 'event') {
        const signature = rskTxHelper.getClient().eth.abi.encodeEventSignature(element);
...
const decodeUnionLog = (log, abiElement) => {
  const decodedLog = rskTxHelper.getClient().eth.abi.decodeLog(
    abiElement.inputs,
    log.data,
    log.topics.slice(1)
  );
