...
const execute = (description, getRskHost, bridgeCallsAllowed) => {
  describe(description, () => {
    ...
    testMethod('getMinimumLockTxValue()', [], '0');
    testMethod('getFederationAddress()', [], '');
    testMethod('getFederationCreationBlockNumber()', [], '0');
    testMethod('getFederationCreationTime()', [], '0');
    testMethod('getFederationSize()', [], '0');
    testMethod('getFederationThreshold()', [], '0');
    testMethod('getFeePerKb()', [], '0');
    testMethod('getPendingFederationSize()', [], '0');
    testMethod('getRetiringFederationSize()', [], '0');
    testMethod('getRetiringFederationThreshold()', [], '0');
    testMethod('getRetiringFederationCreationTime()', [], '0');
    testMethod('getRetiringFederationCreationBlockNumber()', [], '0');
    // TODO: investigate why these two methods are treated as local calls instead of mined calls with "solc": "^0.7.5"
    testMethod('getBtcTxHashProcessedHeight(string)', ['001122334455667788990011223344556677889900112233445566778899aabb'], '0');
    testMethod('getStateForDebugging()', [], '');
  });
}

const testMethod = (methodSignature, args, expectedWhenFail) => {
