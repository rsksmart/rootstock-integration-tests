...
const execute = (description, getRskHost, bridgeCallsAllowed) => {
    describe(description, () => {
        before(async () => {
            ...
            address = await rskClient.send('personal_newAccount', ['']);
            await rskUtils.sendFromCow(
                rskTransactionHelper,
                address,
                Number(btcEthUnitConverter.btcToWeis(INITIAL_RSK_BALANCE_IN_BTC))
            );
            await rskTransactionHelper.unlockAccount(address);
        });
        ...
        it('should create the testing contract', async () => {
            try {
                ...
                const areYouAliveResult = await contractCallsTester.areYouAlive();
...
const testMethod = (bridgeCallsAllowed) => (methodSignature, args, expectedWhenFail) => {
    ...
    describe(methodName, () => {
        ...
        before(async () => {
            ...
            abi = bridge.interface.encodeFunctionData(methodName, args);
        });
        ...
        it('normal call works', async () => {
            try {
                ...
                if (expectedWhenFail === null) {
                    ...
                    // `result` may be a bigint/other non-string type; stringify to compare
                    // against the string sentinel values above (e.g. '0').
                    expect(String(result)).to.not.equal(expectedWhenFail);
                }
            } catch (err) {
