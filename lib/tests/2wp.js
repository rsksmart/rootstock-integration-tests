...
const { sendPeginToActiveFederation,
...
const { ensure0x, removePrefix0x, getHostUrl } = require('../utils');
  ...
  describe(description, () => {
    ...
    describe('Pegin tests', () => {
      ...
      it('should do a basic legacy pegin with the exact minimum value', async () => {
        ...
        const btcPeginTxHash = await sendPeginToActiveFederation(rskTxHelper, btcTxHelper, senderRecipientInfo.btcSenderAddressInfo, satoshisToBtc(peginValueInSatoshis));
        ...
        await assert2wpBalancesAfterSuccessfulPegin(rskTxHelper, btcTxHelper, initial2wpBalances, peginValueInSatoshis);
      ...
      it('should do a basic pegin v1 with the exact minimum value', async () => {
        ...
        const btcPeginTxHash = await sendPeginToActiveFederation(rskTxHelper, btcTxHelper, senderRecipientInfo.btcSenderAddressInfo, Number(satoshisToBtc(peginValueInSatoshis)), peginV1Data);
      ...
      it('should reject and refund a legacy pegin from a multisig account with the value exactly minimum', async () => {
        ...
        const btcPeginTxHash = await sendPeginToActiveFederation(rskTxHelper, btcTxHelper, multisigSenderAddressInfo, Number(satoshisToBtc(peginValueInSatoshis)));
      ...
      it('should do a pegin v1 from multisig with the exact minimum value', async () => {
        ...
        const btcPeginTxHash = await sendPeginToActiveFederation(rskTxHelper, btcTxHelper, multisigSenderAddressInfo, Number(satoshisToBtc(peginValueInSatoshis)), peginV1Data);
      ...
      it('should do a pegin v1 from bech32 sender with the exact minimum value', async () => {
        ...
        const btcPeginTxHash = await sendPeginToActiveFederation(rskTxHelper, btcTxHelper, bech32SenderAddressInfo, Number(satoshisToBtc(peginValueInSatoshis)), peginV1Data);
      ...
      it('should reject and not refund a legacy pegin from a bech32 sender with the value exactly minimum', async () => {
        ...
        const btcPeginTxHash = await sendPeginToActiveFederation(rskTxHelper, btcTxHelper, bech32SenderAddressInfo, Number(satoshisToBtc(peginValueInSatoshis)));
      ...
      it('should reject and refund pegin with multiple OP_RETURN outputs for RSK with value exactly minimum', async () => {
        ...
        const data = [];
        data.push(
          Buffer.from(createPeginV1TxData(senderRecipientInfo.rskRecipientRskAddressInfo.address), 'hex'),
          Buffer.from(createPeginV1TxData(peginV1RskRecipientAddress), 'hex')
        );

        // Act

        const btcPeginTxHash = await sendPeginToActiveFederation(rskTxHelper, btcTxHelper, senderRecipientInfo.btcSenderAddressInfo, satoshisToBtc(peginValueInSatoshis), data);
    ...
    it('should do a pegout with value exactly minimum', async () => {
      ...
      const bridgeStateBeforePegoutCreation = await getBridgeState(getHostUrl(rskTxHelper));
      ...
      const pegoutCreatedCallback = async () => {
        bridgeStateAfterPegoutCreation = await getBridgeState(getHostUrl(rskTxHelper));
      };
      ...
      await assertSuccessfulPegoutEventsAreEmitted(pegoutsEvents, pegoutTransaction.transactionHash, senderRecipientInfo, pegoutValueInSatoshis, bridgeStateBeforePegoutCreation, bridgeStateAfterPegoutCreation);
    ...
    it('should do a pegout and round down the weis to satoshis as expected', async () => {
      ...
      const blockNumberAfterPegoutRelease = await rskTxHelper.getBlockNumber();
      const pegoutsEvents = await getPegoutEventsInBlockRange(rskTxHelper, pegoutTransaction.blockNumber, blockNumberAfterPegoutRelease);
      await assertSuccessfulPegoutEventsAreEmitted(pegoutsEvents, pegoutTransaction.transactionHash, senderRecipientInfo, expectedPegoutValueInSatoshis, bridgeStateBeforePegoutCreation, bridgeStateAfterPegoutCreation, pegoutValueInWeisBN);
    ...
    it('should create multiple pegouts with mixed values same and above minimum', async () => {
      ...
      const pegoutTransaction3 = await sendTxToBridge(rskTxHelper, new BN(satoshisToWeis(pegout3ValueInSatoshis)), senderRecipientInfo3.rskRecipientRskAddressInfo.address);
      ...
      const bridgeStateAfterPegoutRequestsReceived = await getBridgeState(getHostUrl(rskTxHelper));
...
const assertReleaseRequestReceivedEvent = (releaseRequestReceivedEvent, rskSenderAddress, btcRecipientAddress, expectedPegoutValue) => {
  expect(releaseRequestReceivedEvent.arguments.sender).to.be.equal(rskSenderAddress);
  expect(Number(releaseRequestReceivedEvent.arguments.amount)).to.be.equal(Number(expectedPegoutValue));
...
const assertSuccessfulPegoutEventsAreEmitted = async (pegoutsEvents, pegoutRequestReceivedTransactionHash, senderRecipientInfo, pegoutValueInSatoshis, bridgeStateBeforePegoutCreation, bridgeStateAfterPegoutCreation, pegoutValueInWeis) => {
...
const assertExpectedReleaseRequestRejectedEventIsEmitted = async (rskSenderAddress, expectedPegoutValue, rejectionReason) => {
  const rskSenderAddressChecksummed = rskTxHelper.getClient().utils.toChecksumAddress(ensure0x(rskSenderAddress));
  const expectedEvent = createExpectedReleaseRequestRejectedEvent(rskSenderAddressChecksummed, expectedPegoutValue, rejectionReason);
...
const assertPegoutTransactionCreatedEvent = (pegoutTransactionCreatedEvent, btcTransaction, bridgeStateBeforePegoutRequestsReceived) => {
  ...
  expect(btcTransaction.getId()).to.be.equal(removePrefix0x(pegoutTransactionCreatedEvent.arguments.btcTxHash));
  for (let index = 0; index < btcTransaction.ins.length; index++) {
    ...
    const bridgeUtxo = bridgeStateBeforePegoutRequestsReceived.activeFederationUtxos.find(utxo => {
