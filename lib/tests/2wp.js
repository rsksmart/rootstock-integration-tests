...
const execute = (description, getRskHost) => {
  ...
  describe(description, () => {
    ...
    it('should do a pegout with value exactly minimum', async () => {
      ...
      await assertSuccessfulPegoutEventsAreEmitted(pegoutsEvents, pegoutTransaction.transactionHash, senderRecipientInfo, satoshisToWeis(pegoutValueInSatoshis), pegoutValueInSatoshis, bridgeStateBeforePegoutCreation, bridgeStateAfterPegoutCreation);
    ...
    it('should reject and not refund a pegout when a contract is trying to execute it', async () => {
      ...
      const expectedEvent = createExpectedReleaseRequestRejectedEvent(contractAddressChecksummed, satoshisToWeis(pegoutValueInSatoshis), PEGOUT_REJECTION_REASONS.CALLER_CONTRACT);
    ...
    it('should do a pegout and round down the weis to satoshis as expected', async () => {
      ...
      const blockNumberAfterPegoutRelease = await rskTxHelper.getBlockNumber();
      const pegoutsEvents = await getPegoutEventsInBlockRange(rskTxHelper, pegoutTransaction.blockNumber, blockNumberAfterPegoutRelease);
      await assertSuccessfulPegoutEventsAreEmitted(pegoutsEvents, pegoutTransaction.transactionHash, senderRecipientInfo, pegoutValueInWeisBN, expectedPegoutValueInSatoshis, bridgeStateBeforePegoutCreation, bridgeStateAfterPegoutCreation);
    ...
    it('should create multiple pegouts with mixed values same and above minimum', async () => {
      ...
      const bridgeStateBeforePegoutRequestsReceived = await getBridgeState(rskTxHelper.getClient());
      ...
      assertReleaseRequestReceivedEvent(releaseRequestReceivedEvent1, rskSender1Address, senderRecipientInfo1.btcSenderAddressInfo.address, satoshisToWeis(pegout1ValueInSatoshis));
      ...
      assertReleaseRequestReceivedEvent(releaseRequestReceivedEvent2, rskSender2Address, senderRecipientInfo2.btcSenderAddressInfo.address, satoshisToWeis(pegout2ValueInSatoshis));
      ...
      assertReleaseRequestReceivedEvent(releaseRequestReceivedEvent3, rskSender3Address, senderRecipientInfo3.btcSenderAddressInfo.address, satoshisToWeis(pegout3ValueInSatoshis));
      ...
      assertPegoutTransactionCreatedEvent(pegoutTransactionCreatedEvent, btcTransaction, bridgeStateBeforePegoutRequestsReceived);
...
const assertReleaseRequestReceivedEvent = (releaseRequestReceivedEvent, rskSenderAddress, btcRecipientAddress, pegoutValueInWeisBN) => {
  expect(releaseRequestReceivedEvent.arguments.sender).to.be.equal(rskSenderAddress);
  expect(Number(releaseRequestReceivedEvent.arguments.amount)).to.be.equal(Number(pegoutValueInWeisBN));
...
const assertReleaseRequestedEvent = (releaseRequestedEvent, pegoutCreationRskTransactionHash, btcTxHash, pegoutValueInSatoshis) => {
  ...
  expect(releaseRequestedEvent.arguments.amount).to.be.equal(pegoutValueInSatoshis.toString());
};
...
const assertSuccessfulPegoutEventsAreEmitted = async (pegoutsEvents, pegoutRequestReceivedTransactionHash, senderRecipientInfo, pegoutValueInWeisBN, pegoutValueInSatoshis, bridgeStateBeforePegoutCreation, bridgeStateAfterPegoutCreation) => {
  ...
  assertReleaseRequestReceivedEvent(releaseRequestReceivedEvent, rskSenderAddress, senderRecipientInfo.btcSenderAddressInfo.address, pegoutValueInWeisBN);
...
const assertExpectedReleaseRequestRejectedEventIsEmitted = async (rskSenderAddress, amountInSatoshis, rejectionReason) => {
  ...
  const expectedEvent = createExpectedReleaseRequestRejectedEvent(rskSenderAddressChecksummed, satoshisToWeis(amountInSatoshis), rejectionReason);
