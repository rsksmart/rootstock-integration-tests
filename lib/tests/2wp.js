    ...
    describe('Pegin tests', () => {
      ...
      it('should reject and refund pegin with OP_RETURN output to RSK with invalid payload with value exactly minimum', async () => {
        ...
        const btcPeginTxHash = await sendPeginToActiveFederation(rskTxHelper, btcTxHelper, senderRecipientInfo.btcSenderAddressInfo, satoshisToBtc(peginValueInSatoshis), peginV1Data);
         ...
         console.log(`Pegin tx hash: ${btcPeginTxHash}`);
...
const assert2wpBalanceIsUnchanged = async (initial2wpBalances) => {
  const final2wpBalances = await get2wpBalances(rskTxHelper, btcTxHelper);
  console.log('final2wpBalances: ', final2wpBalances);
  console.log('initial2wpBalances: ', initial2wpBalances);
...
const assertPegoutTransactionCreatedEvent = (pegoutTransactionCreatedEvent, btcTransaction, bridgeStateBeforePegoutRequestsReceived) => {
  ...
  btcTransaction.ins.forEach((input, index) => {
