...
const execute = (description, fullExecution = false) => {
    describe(description, () => {
        before(async () => {
            ...
            federationAddress = await bridge.getFederationAddress();
            ...
            if (fullExecution) {
                ...
                it('should reject and refund a legacy pegin from a multisig account with the value exactly minimum', async () => {
                    ...
                    const btcTxHashProcessedHeight = Number(
                        await bridge.getBtcTxHashProcessedHeight(btcPeginTxHash)
                    );
        ...
        if (fullExecution) {
            ...
            it('should reject and refund a pegout when fee per kb is above value', async () => {
                ...
                const initialFeePerKb = Number(await bridge.getFeePerKb());
            ...
            it('should reject and not refund a pegout when a contract is trying to execute it', async () => {
                ...
                const contractAddressChecksummed = ethers.getAddress(
                    ensure0x(callReleaseBtcContract.target)
                );
            ...
            it('should create multiple pegouts with mixed values same and above minimum', async () => {
                ...
                const rskSender1Address = ethers.getAddress(
                ...
                const rskSender2Address = ethers.getAddress(
                ...
                const rskSender3Address = ethers.getAddress(
...
const assertExpectedPeginBtcEventIsEmitted = async (
    ...
    const recipient1RskAddressChecksumed = ethers.getAddress(ensure0x(rskRecipientAddress));
...
const assertPeginTxHashNotProcessed = async (btcPeginTxHash) => {
    const isBtcTxHashAlreadyProcessed = await bridge.isBtcTxHashAlreadyProcessed(btcPeginTxHash);
...
const assertSuccessfulPegoutEventsAreEmitted = async (
    ...
    const rskSenderAddress = ethers.getAddress(
        ensure0x(senderRecipientInfo.rskRecipientRskAddressInfo.address)
    );
...
const assertExpectedReleaseRequestRejectedEventIsEmitted = async (
    ...
    const rskSenderAddressChecksummed = ethers.getAddress(ensure0x(rskSenderAddress));
    const expectedEvent = createExpectedReleaseRequestRejectedEvent(
...
const getReleaseRequestRejectedEventFromContractCallTxReceipt = (contractCallTxReceipt) => {
    ...
    const logData = contractCallTxReceipt.logs[0];
    const releaseRequestRejectedAbiElement = precompiledAbis.bridge.abi.find(
        (item) => item.type === 'event' && item.name === PEGOUT_EVENTS.RELEASE_REQUEST_REJECTED.name
    );
