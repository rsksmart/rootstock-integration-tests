...
const execute = (description, getRskHost) => {
  ...
  describe(description, () => {
    ...
    it('should reject and refund a pegout when fee per kb is above value', async () => {
      ...
      await assertExpectedReleaseRequestRejectedEventIsEmitted(senderRecipientInfo.rskRecipientRskAddressInfo.address, btcToSatoshis(pegoutValueInRbtc), PEGOUT_REJECTION_REASONS.FEE_ABOVE_VALUE);
    ...
    it.skip('should create multiple pegouts with values exactly minimum', async () => {
      ...
      const peginValueInSatoshis = btcToSatoshis(0.5);
       // Create a pegin for the senders to ensure there is enough funds to pegout and because this is the natural process
      const btcPeginTxHash1 = await sendPegin(rskTxHelper, btcTxHelper, senderRecipientInfo1.btcSenderAddressInfo, satoshisToBtc(peginValueInSatoshis));
      await ensurePeginIsRegistered(rskTxHelper, btcPeginTxHash1);

      const senderRecipientInfo2 = await createSenderRecipientInfo(rskTxHelper, btcTxHelper);
      const btcPeginTxHash2 = await sendPegin(rskTxHelper, btcTxHelper, senderRecipientInfo2.btcSenderAddressInfo, satoshisToBtc(peginValueInSatoshis));

      await ensurePeginIsRegistered(rskTxHelper, btcPeginTxHash2);

      const senderRecipientInfo3 = await createSenderRecipientInfo(rskTxHelper, btcTxHelper);
      const btcPeginTxHash3 = await sendPegin(rskTxHelper, btcTxHelper, senderRecipientInfo3.btcSenderAddressInfo, satoshisToBtc(peginValueInSatoshis));

      await ensurePeginIsRegistered(rskTxHelper, btcPeginTxHash3);
      ...
      // Waiting for our 3 txs to reach the mempool before trying to mine them.
      // + 3 because we are sending 3 pegouts.
      const expectedRskMempoolTxHashesSize = initialRskMempoolTxHashesSize + 3;
      while((await getRskMempoolTransactionHashes(rskTxHelper)).length < expectedRskMempoolTxHashesSize) {
        await wait(500);
      }
      ...
      const addSignatureEvent1 = pegoutsEvents[6];
      assertAddSignatureEvent(addSignatureEvent1, releaseRequestedEvent);

      const addSignatureEvent2 = pegoutsEvents[7];
      assertAddSignatureEvent(addSignatureEvent2, releaseRequestedEvent);

      const addSignatureEvent3 = pegoutsEvents[8];
      assertAddSignatureEvent(addSignatureEvent3, releaseRequestedEvent);

      //

      const releaseBtcEvent = pegoutsEvents[9];
      assertReleaseBtcEvent(releaseBtcEvent, releaseRequestedEvent);

      // Summing the pegout values, first converting them to satoshis to avoid js decimal issues.
      const totalAmountOfPegoutsInRbtc = Number(satoshisToBtc(totalPegoutValueInSatoshis));
      await assert2wpBalanceAfterSuccessfulPegout(initial2wpBalances, totalAmountOfPegoutsInRbtc);
      ...
      const serder1Utxo = getAddressUtxo(releaseBtcTransaction.outs, senderRecipientInfo1.btcSenderAddressInfo.address);
      const actualPegoutValueReceivedInSatoshis = serder1Utxo.value;
      expect(finalSenderAddressBalanceInSatoshis1).to.be.equal(initialSender1AddressBalanceInSatoshis + actualPegoutValueReceivedInSatoshis);
      ...
      const actualPegoutValueReceivedInSatoshis2 = serder2Utxo.value;
      expect(finalSenderAddressBalanceInSatoshis2).to.be.equal(initialSender2AddressBalanceInSatoshis + actualPegoutValueReceivedInSatoshis2);
      ...
      const actualPegoutValueReceivedInSatoshis3 = serder3Utxo.value;
      expect(finalSenderAddressBalanceInSatoshis3).to.be.equal(initialSender3AddressBalanceInSatoshis + actualPegoutValueReceivedInSatoshis3);
...
const assertSuccessfulPegoutEventsAreEmitted = async (pegoutsEvents, pegoutRequestReceivedTransactionHash, senderRecipientInfo, pegoutValueInRbtc, bridgeStateAfterPegoutCreation) => {
  ...
  assertReleaseRequestReceivedEvent(releaseRequestReceivedEvent, rskSenderAddress, senderRecipientInfo.btcSenderAddressInfo.address, btcToSatoshis(pegoutValueInRbtc));

  // release_requested event
  const pegoutCreationRskTransactionHash = ensure0x(pegoutWaitingForConfirmationWhenPegoutWasCreated.rskTxHash.padStart(64, '0'));
  const releaseRequestedEvent = pegoutsEvents[1];
  assertReleaseRequestedEvent(releaseRequestedEvent, pegoutCreationRskTransactionHash, btcTransaction.getId(), btcToSatoshis(pegoutValueInRbtc));
