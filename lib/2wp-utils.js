...
const assertRefundUtxosSameAsPeginUtxos = async (
    ...
    const federationAddress = await bridge.getFederationAddress();
...
const sendTxToBridge = async (rskTxHelper, amountInWeisBN, rskFromAddress, mine = true) => {
    ...
    return await rskTxHelper.getTxReceipt(txHash);
};
...
const getActiveFederationUtxos = async (rskTxHelper) => {
    const activeUtxosRlpEncoded = await getStorageBytesAt(
...
const ensurePeginIsRegistered = async (rskTxHelper, peginBtcTxHash, expectedUtxosCount = 1) => {
    ...
    const bridge = await getBridge(rskTxHelper.getClient());
    const isBtcTxHashAlreadyProcessed = await bridge.isBtcTxHashAlreadyProcessed(peginBtcTxHash);
