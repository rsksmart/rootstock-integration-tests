...
const ensurePeginIsRegistered = async (rskTxHelper, peginBtcTxHash, expectedUtxosCount = 1) => {
    ...
    const check = async (utxoIsRegistered, currentAttempts) => {
        logger.debug(
            `[${ensurePeginIsRegistered.name}::${check.name}] Attempting to find the pegin ${peginBtcTxHash} in the bridge. Attempt ${currentAttempts} out of ${MAX_ATTEMPTS}`
        );
        if (!utxoIsRegistered) {
    ...
    if (utxoIsRegisteredInTheBridge && isBtcTxHashAlreadyProcessed) {
        logger.debug(
            `[${ensurePeginIsRegistered.name}] Found pegin ${peginBtcTxHash} registered in the bridge.`
        );
...
const disableWhitelisting = async (rskTxHelper, btcTxHelper, blockDelay = 1) => {
    const bridge = await getBridge(rskTxHelper.getClient());
    const unlocked = await getUnlockedAddress(
        rskTxHelper,
        WHITELIST_CHANGE_PK,
    ...
    await sendTxWithCheck(
        rskTxHelper,
        disableLockWhitelistMethod,
        WHITELIST_CHANGE_ADDR,
        disableResultCallback
    );
    if (blockDelay > 0) {
