 ...
 */
const assertRefundUtxosSameAsPeginUtxos = async (
    rskTxHelper,
    btcTxHelper,
    peginTxHash,
    refundAddress
    ...
    const outputsForFederation = peginTx.outs.filter(
        (output) => btcTxHelper.getOutputAddress(output.script) === federationAddress
    );
...
const getActiveFederationUtxos = async (rskTxHelper) => {
    const activeUtxosRlpEncoded = await rskTxHelper
        .getClient()
        .rsk.getStorageBytesAt(BRIDGE_ADDRESS, newFederationBtcUTXOSStorageIndex);
...
const isUtxoRegisteredInBridge = async (rskTxHelper, peginTxHash, expectedUxtosCount = 1) => {
    ...
    return (
        totalUtxos.filter((utxo) => utxo.btcTxHash === peginTxHash).length === expectedUxtosCount
    );
};
...
const sendPegin = async (
    ...
    const recipientsTransferInformation = outputAmountsInBtc.map((amount) => ({
        recipientAddress: federationAddress,
        amountInBtc: amount,
    }));

    const peginBtcTxHash = await btcTxHelper.transferBtc(
        btcSenderAddressInformation,
        recipientsTransferInformation,
        data
    );
...
const sendPeginToActiveAndRetiringFederations = async (
    rskTxHelper,
    btcTxHelper,
    btcSenderAddressInformation,
    outputAmountsInBtcForActiveFederation,
    outputAmountsInBtcForRetiringFederation,
    data
) => {
    ...
    const recipientsTransferInformation = outputAmountsInBtcForActiveFederation.map((amount) => ({
        recipientAddress: federationAddress,
        amountInBtc: Number(amount),
    }));
    const recipientsTransferInformationRetiring = outputAmountsInBtcForRetiringFederation.map(
        (amount) => ({ recipientAddress: retiringFederationAddress, amountInBtc: Number(amount) })
    );
 ...
 * @returns {Promise<string>} the pegin btc tx hash
 */
const sendPeginToActiveFederation = async (
    rskTxHelper,
    btcTxHelper,
    btcSenderAddressInformation,
    outputAmountsInBtc,
 ...
 * @returns {Promise<string>} the pegin btc tx hash
 */
const sendPeginToRetiringFederation = async (
    rskTxHelper,
    btcTxHelper,
    btcSenderAddressInformation,
    outputAmountsInBtc,
 ...
 */
const ensureBtcTxIsRegistered = async (rskTxHelper, peginBtcTxHash, expectedUtxosCount = 1) => {
    ...
    const check = async (utxoIsRegistered, currentAttempts) => {
        logger.debug(
            `[${ensureBtcTxIsRegistered.name}::${check.name}] Attempting to find the pegin ${peginBtcTxHash} in the bridge. Attempt ${currentAttempts} out of ${MAX_ATTEMPTS}`
        );
        if (!utxoIsRegistered) {
    ...
    const { result: utxoIsRegisteredInTheBridge } = await retryWithCheck(
        method,
        check,
        MAX_ATTEMPTS,
    ...
    const isBtcTxHashAlreadyProcessed = await bridge.methods
        .isBtcTxHashAlreadyProcessed(peginBtcTxHash)
        .call();

    if (utxoIsRegisteredInTheBridge && isBtcTxHashAlreadyProcessed) {
        logger.debug(
            `[${ensureBtcTxIsRegistered.name}] Found pegin ${peginBtcTxHash} registered in the bridge.`
        );
    ...
    throw new Error(
        `Could not find the pegin registered in the bridge after ${MAX_ATTEMPTS} attempts`
    );
};
 ...
 */
const donateToBridge = async (
    rskTxHelper,
    btcTxHelper,
    donatingBtcAddressInformation,
    amountInBtc
) => {
    const data = [];
    ...
    await ensureBtcTxIsRegistered(rskTxHelper, peginBtcTxHash);
    return peginBtcTxHash;
};
 ...
 */
const createSenderRecipientInfo = async (
    rskTxHelper,
    btcTxHelper,
    type = 'legacy',
    initialAmountToFundInBtc = 1
) => {
    const btcSenderAddressInfo = await btcTxHelper.generateBtcAddress(type);
    const rskRecipientRskAddressInfo = getDerivedRSKAddressInformation(
 ...
 * @returns {BridgeEvent}
 */
const createExpectedPeginBtcEvent = (
    rskRecipientRskAddress,
    btcPeginTxHash,
...
const get2wpBalances = async (rskTxHelper, btcTxHelper) => {
    ...
    const federationAddress = await bridge.methods.getFederationAddress().call();
    const retiringFederationAddress = await bridge.methods.getRetiringFederationAddress().call();
    const federationAddressBalanceInSatoshis = await getBtcAddressBalanceInSatoshis(
        btcTxHelper,
        federationAddress
    );
    const retiringFederationAddressBalanceInSatoshis = await getBtcAddressBalanceInSatoshis(
        btcTxHelper,
        retiringFederationAddress
    );
 ...
 * @returns {Promise<string>} the pegin tx hash
 */
const fundRskAccountThroughAPegin = async (
    rskTxHelper,
    btcTxHelper,
    btcSenderAddressInfo,
    amountInBtcToFundInBtc = DEFAULT_RSK_ADDRESS_FUNDING_IN_BTC
) => {
    const btcPeginTxHash = await sendPeginToActiveFederation(
        rskTxHelper,
        btcTxHelper,
        btcSenderAddressInfo,
        amountInBtcToFundInBtc
    );
    await ensureBtcTxIsRegistered(rskTxHelper, btcPeginTxHash);
    return btcPeginTxHash;
};

const createExpectedReleaseRequestRejectedEvent = (
    rskSenderAddress,
    amountInSatoshis,
    rejectionReason
) => {
