...
// bridge-transaction-parser v3 expects an ethers provider (not a web3 client).
// Cache one per host to avoid re-creating it for every block scanned.
const ethersProvidersByHost = {};
const getEthersProvider = (rskTxHelper) => {
    const host = getRskHost(rskTxHelper);
    if (!ethersProvidersByHost[host]) {
        ethersProvidersByHost[host] = new JsonRpcProvider(host);
    }
    return ethersProvidersByHost[host];
};

const findBridgeTransactionsInThisBlock = async (rskTxHelper, blockHashOrBlockNumber) => {
    const bridgeTxParser = new BridgeTransactionParser(
        getEthersProvider(rskTxHelper)
    );
