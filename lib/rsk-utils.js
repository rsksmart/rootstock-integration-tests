 ...
 */
const getRskHost = (rskTxHelper) => rskTxHelper.getClient().currentProvider.host;
...
const increaseBlockToNextPegoutHeight = async (rskTransactionHelpers) => {
    ...
    const nextPegoutCreationBlockNumber = await bridge.methods
...
const triggerReleaseUntilPipelineDrained = async (
    rskTxHelpers,
    btcTxHelper,
    maxReleaseCycles = 20
) => {
    const rskTxHelper = rskTxHelpers[0];
    const isPegoutPipelineDrained = async () => {
        const bridgeState = await getBridgeState(getRskHost(rskTxHelper));
        return (
            bridgeState.pegoutRequests.length === 0 &&
            bridgeState.pegoutsWaitingForConfirmations.length === 0 &&
            bridgeState.pegoutsWaitingForSignatures.length === 0
        );
    };
    let releaseCycles = 0;
    while (!(await isPegoutPipelineDrained()) && releaseCycles < maxReleaseCycles) {
        await triggerRelease(rskTxHelpers, btcTxHelper, {}, releaseCycles === 0);
        releaseCycles++;
    }
    return { drained: await isPegoutPipelineDrained(), releaseCycles };
};
...
const sendTransaction = async (rskTxHelper, method, from, valueInWeis = 0, gas = 100000) => {
    ...
    // Any path that exits before `return await txReceiptPromise` (tx-hash error, mempool
    // RPC failure, fail-fast throw below) would otherwise leave its rejection unhandled.
    // `.catch()` returns a new promise, so rejections still propagate to the caller on
    // the awaited return below.
    txReceiptPromise.catch(() => {});

    // The node emits the tx hash once it accepts the tx. Wait for that specific tx to be
    // in the submitter node's mempool before mining, since `method` may target any
    // contract, not just the bridge.
    const txHash = await new Promise((resolve, reject) => {
        txReceiptPromise.once('transactionHash', resolve);
        txReceiptPromise.once('error', reject);
    });

    // Fail fast instead of mining if the tx never reaches the mempool, using the same
    // attempt budget as the previous generic mempool wait (10 attempts).
    const isTxInTheMempool = await waitForRskTxToBeInTheMempool(rskTxHelper, txHash, 10);
    if (!isTxInTheMempool) {
        throw new Error(
            `The tx (${txHash}) was not found in the mempool nor mined in a block. It may have been rejected by the node.`
        );
    }
...
// Cache one provider per host to avoid re-creating it for every block scanned
const ethersProvidersByHost = new Map();
const getEthersProvider = (rskTxHelper) => {
    const host = getRskHost(rskTxHelper);
    let provider = ethersProvidersByHost.get(host);
    if (!provider) {
        provider = new JsonRpcProvider(host);
        ethersProvidersByHost.set(host, provider);
    }
    return provider;
};

const findBridgeTransactionsInThisBlock = async (rskTxHelper, blockHashOrBlockNumber) => {
...
const voteFeePerKbChange = async (
    ...
    await sendTxWithCheck(
        rskTxHelper,
        bridge.methods.voteFeePerKbChange(feePerKbInSatoshis),
        FEE_PER_KB_CHANGER_ADDRESS,
        (result) => {
...
const decodeLog = (rskClient, log, abiElement) => {
    const decodedLog = rskClient.eth.abi.decodeLog(
        abiElement.inputs,
        log.data,
        log.topics.slice(1)
    );
