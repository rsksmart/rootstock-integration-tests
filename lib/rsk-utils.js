 ...
 * @param {Number} blockNumber block to wait for
 * @param {Number} waitTime defaults to 200 milliseconds. Time to wait before checking for the block on every iteration
 * @param {Number} maxAttempts defaults to 6000 milliseconds. Attempts to try to get the indicated block number
 * @returns {Promise<Number>} the latest block number that we were waiting for
 */
const waitForBlock = (rskClient, blockNumber, waitTime = 200, maxAttempts = 600) => {
  return new Promise((resolve, reject) => {
    let attempts = 0;
    const checkBlockNumber = () => {
      rskClient.eth.getBlockNumber().then(latestBlockNumber => {
        if (latestBlockNumber >= blockNumber) {
          return resolve(latestBlockNumber);
        }
        if (attempts++ === maxAttempts) {
          return reject(new Error(`Block number ${blockNumber} never reached, last seen was ${latestBlockNumber}`));
        };
...
const increaseBlockToNextPegoutHeight = async (rskTransactionHelpers) => {
  ...
  const bridge = getBridge(rskTransactionHelper.getClient(), await getLatestActiveForkName());
...
/** Activates the given fork if it's not already active by mining blocks until reach the activation block
 * @param {Runners.common.forks[fork]} fork 
 * @returns {Promise<void>}
 */
const activateFork = async (fork) => {

  const initialForkActivationState = await fork.isAlreadyActive();
  expect(initialForkActivationState, `Fork ${fork.name} already active`).to.be.false;

  const rskTxHelpers = getRskTransactionHelpers();
  const blockNum = await rskTxHelpers[0].getBlockNumber();
  const activationBlock = fork.activationHeight;

  const differenceBlockNumbers = activationBlock - blockNum + 1; // + 1 to be above the activation block
  await mineAndSync(rskTxHelpers, differenceBlockNumbers);
  const finalForkActivationState = await fork.isAlreadyActive();
  expect(finalForkActivationState, `Fork ${fork.name} is not yet active`).to.be.true;

};

/**
 * Get the name of latest fork 
 * @returns {string} The name of the latest fork
 */
const getLatestForkName = () => {
