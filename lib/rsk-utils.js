...
const { HSM_DIFFICULTY_TARGET, waitForHsmsToBeSynchedToThisBlock, isRunningHsms } = require('./federators-utils');
  ...
  return new Promise((resolve, reject) => {
    ...
    const checkBlockNumber = () => {
      rskClient.eth.getBlockNumber().then(newLatestBlockNumber => {
        ...
        console.log(`[waitForBlock] error attempts remaining: ${maxAttemptsOnError}`);
        if (maxAttemptsOnError-- === 0) {
          return reject(new Error(`[waitForBlock] Error getting block number from rsk client: ${error.message}`));
        }
        setTimeout(checkBlockNumber, waitTime);
      });
    };
 ...
 */
const waitAndUpdateBridge = async (rskTxHelper, timeInMilliseconds = 1000, shouldMineAndSync = true) => {
...
const waitForRskMempoolToGetThisCountOfAddSignatureTxs = async (rskTxHelper, expectedCount, maxAttempts = 24, checkEveryMilliseconds = 2000) => {
  return waitForRskMempoolToGetThisCountOfThisTxType(rskTxHelper, BRIDGE_TX_TYPES.ADD_SIGNATURE, expectedCount, maxAttempts, checkEveryMilliseconds);
};

const waitForRskMempoolToGetThisCountOfThisTxType = async (rskTxHelper, txType, expectedCount, maxAttempts = 24, checkEveryMilliseconds = 2000) => {

  const method = async () => {

    const mempoolTxs = await getRskMempoolTransactionsToTheBridge(rskTxHelper, true);
    const txsToBridge = mempoolTxs.filter(tx => tx.input && tx.input.startsWith(txType.methodSelector));

    if(txsToBridge.length >= expectedCount) {
      logger.debug(`[${waitForRskMempoolToGetThisCountOfThisTxType.name}] Found ${txsToBridge.length} ${txType.name} txs in the rsk mempool, expected at least ${expectedCount}`);
      return true;
    }
    logger.debug(`[${waitForRskMempoolToGetThisCountOfThisTxType.name}] Found ${txsToBridge.length} ${txType.name} txs in the rsk mempool, expected at least ${expectedCount}. Will keep retrying until it is found or it reaches the max attempts to find it`);
    return false;
  };

  const check = async (expectedCountOfSignatureTxsFound, currentAttempts) => {
    logger.debug(`[${waitForRskMempoolToGetThisCountOfThisTxType.name}::${check.name}] Attempting to find ${expectedCountOfSignatureTxsFound} ${txType.name} txs in the rsk mempool. Found ${currentAttempts} so far. Attempt ${currentAttempts} out of ${maxAttempts}`);
...
const waitForRskMempoolToGetThisCountOfRegisterBtcTx = async (rskTxHelper, expectedCount, maxAttempts = 24, checkEveryMilliseconds = 2000) => {
  return waitForRskMempoolToGetThisCountOfThisTxType(rskTxHelper, BRIDGE_TX_TYPES.REGISTER_BTC_TRANSACTION, expectedCount, maxAttempts, checkEveryMilliseconds);
};
...
const waitForRskTxToBeInTheMempool = async (rskTxHelper, txHash, maxAttempts = 3, checkEveryMilliseconds = 500) => {
  ...
  const method = async () => {
    ...
    if(isTxInTheMempool) {
      logger.debug(`[${waitForRskTxToBeInTheMempool.name}::${method.name}] The tx (${txHash}) is in the mempool`);
      return true;
    }
    ...
    if(isTxAlreadyMined) {
      logger.debug(`[${waitForRskTxToBeInTheMempool.name}::${method.name}] The tx (${txHash}) is already mined in a block`);
      return true;
    }

    logger.debug(`[${waitForRskTxToBeInTheMempool.name}::${method.name}] The tx (${txHash}) is not in the mempool nor in a block yet. Will keep retrying until it is in the mempool, block, or it reaches the max attempts to find it`);
 ...
 */
const triggerRelease = async (rskTransactionHelpers, btcClient, callbacks = {}, shouldIncreaseBlockToNextPegoutHeight = true) => {

  const rskTxHelper = rskTransactionHelpers[0];

  const latestBlockNumber = await rskTxHelper.getBlockNumber();

  console.log('block number before increaseBlockToNextPegoutHeight: ', latestBlockNumber);
  ...
  console.log('block number after increaseBlockToNextPegoutHeight: ', latestBlockNumber1);
  ...
  console.log('initialBridgeState.pegoutsWaitingForConfirmations.length: ', initialBridgeState.pegoutsWaitingForConfirmations.length);
  console.log('initialBridgeState.pegoutsWaitingForSignatures.length: ', initialBridgeState.pegoutsWaitingForSignatures.length);
  console.log('initialBridgeState.pegoutsWaitingForSignatures: ', initialBridgeState.pegoutsWaitingForSignatures);
  ...
  if(isRunningHsms()) {
    ...
    console.log('Expecting hsms to be synced to this block number: ', latestBlockNumber);
  ...
  const bridgeStateAfterMove = await getBridgeState(rskTxHelper.getClient());

  console.log('bridgeStateAfterMove.pegoutsWaitingForConfirmations.length: ', bridgeStateAfterMove.pegoutsWaitingForConfirmations.length);
  console.log('bridgeStateAfterMove.pegoutsWaitingForSignatures.length: ', bridgeStateAfterMove.pegoutsWaitingForSignatures.length);
  console.log('bridgeStateAfterMove.pegoutsWaitingForSignatures: ', bridgeStateAfterMove.pegoutsWaitingForSignatures);
  ...
  const method = async () => {
    ...
    console.log('currentBridgeState.pegoutsWaitingForSignatures.length: ', currentBridgeState.pegoutsWaitingForSignatures.length);
    ...
    await waitForHsmsToBeSynchedToThisBlock(rskTxHelper, latestBlockNumber - HSM_DIFFICULTY_TARGET);
