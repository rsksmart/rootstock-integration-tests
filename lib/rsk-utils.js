...
const sendFromCow = async (rskTxHelper, recipientAddress, amountInWeis) => {
    ...
    // `gas` is set explicitly: unlike web3.js, this raw `eth_sendTransaction` call does no
    // client-side gas estimation, so it's left entirely to the node's own default otherwise.
    ...
    expect(txReceipt && txReceipt.status, `Transfer from cow to ${recipientAddress} failed`).to.be
        .true;
