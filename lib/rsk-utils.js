...
const waitForSync = async (rskTransactionHelpers) => {
    ...
    const perNodeBlockNumbers = await Promise.all(
        rskTransactionHelpers.map((rskTxHelper) => rskTxHelper.getBlockNumber())
    );
    perNodeBlockNumbers.forEach((blockNumber, index) => {
        const message = `[${waitForSync.name}] ${getRskHost(rskTransactionHelpers[index])} is at block ${blockNumber}, waiting to reach block ${latestBlockNumber}`;
        logger.debug(message);
        // logger.debug is silenced in CI (LOG_LEVEL defaults to 'info'); console.log guarantees
        // this still shows up in the GitHub Actions job output.
        console.log(message);
    });
        ...
        const checkBlockNumber = () => {
            rskClient.eth
                ...
                .then((newLatestBlockNumber) => {
                    console.log(
                        `[waitForBlock] ${rskClient.currentProvider.host} is at block ${newLatestBlockNumber} (attempt ${attempts}/${maxAttempts}), waiting to reach block ${blockNumber}`
                    );
