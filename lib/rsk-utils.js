...
const { wait, retryWithCheck, removePrefix0x, ensure0x, getHostUrl } = require('./utils');
...
const triggerRelease = async (rskTransactionHelpers, btcClient, callbacks = {}, shouldIncreaseBlockToNextPegoutHeight = true) => {
  ...
  const method = async () => {
    const currentBridgeState = await getBridgeState(getHostUrl(rskTxHelper));
...
const findEventInBlock = async (rskTxHelper, eventName, fromBlockHashOrBlockNumber, toBlockHashOrBlockNumber, check) => {
  if(typeof eventName !== 'string') {
    throw new TypeError('`eventName` parameter must be a string. It is of type: ' + typeof eventName);
  }
...
const getNewFundedRskAddress = async (rskTxHelper, fundingAmountInRbtc = DEFAULT_RSK_ADDRESS_FUNDING_IN_BTC) => {
  const account = rskTxHelper.getClient().eth.accounts.create();
