...
class TcpSignerRunner {
    ...
    async start() {
        ...
        const args = [...this.extraArgs, `-p${this.port}`, `--key=key.json`];
        ...
        const scriptPath = runningInMacOS
            ? path.resolve(tcpSignerExecutionPath, 'run.sh')
            : path.resolve(tcpSignerExecutionPath, 'entrypoint.sh');
    ...
    async waitForReady() {
        ...
        while (attempts < maxAttempts) {
            try {
                ...
                // NOSONAR: Exceptions are expected during retry logic; connection errors are handled by incrementing attempts and waiting before retry
                attempts++;
    ...
    async getVersion() {
        ...
        try {
            ...
            throw new Error(
                `Failed to get version from instance "${this.dockerContainerName}": ${err.message}`
            );
        }
    }
    ...
    async getPublicKey(keyId) {
        ...
        try {
            ...
            throw new Error(
                `Failed to get public key from instance "${this.dockerContainerName}": ${err.message}`
            );
        }
    }
