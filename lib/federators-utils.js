...
const waitForHsmsToBeSynchedToThisBlock = async (rskTxHelper, blockNumber, maxAttempts = 12) => {
    const hsms = Runners.fedRunners
        .filter((fedRunner) => fedRunner.hsm)
        .map((fedRunner) => fedRunner.hsm);
    ...
    while (!allHsmsAreSyncedToExpectedBlock) {
        ...
        try {
            const hsmsBestBlockNumbers = await Promise.all(
                hsms.map((hsm) => getHsmBestBlockNumber(rskTxHelper, hsm))
            );

            allHsmsAreSyncedToExpectedBlock = hsmsBestBlockNumbers.every(
                (hsmBestBlockNumber) => hsmBestBlockNumber >= blockNumber
            );
            ...
            const isSynching = !previousHsmsBestBlockNumbers.every(
                (blockNumber, index) => hsmsBestBlockNumbers[index] === blockNumber
            );
