...
const deployLiquidityBridgeContract = async (host = null) => {
    ...
    await sendFromCow(
        ...
        fromAddress,
        Number(btcToWeis(INITIAL_RSK_BALANCE_IN_BTC))
    );
...
const getDerivationHash = async (preHash, userBtcRefundAddress, liquidityProviderBtcAddress) => {
    ...
    let derivationHash = await instance.methods
        .getDerivationHash(preHash, userBtcRefundAddress, liquidityProviderBtcAddress)
        .call();
