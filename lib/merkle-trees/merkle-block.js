...
class MerkleBlock {
    static fromBitcore(merkleRoot, merkleBlock) {
        return new MerkleBlock(
            merkleRoot,
            merkleBlock.hashes.map(strings_1.hexReverse),
            utils_1.flagsToBits(merkleBlock.flags),
    ...
    reduce(leafCallback, innerCallback) {
        let calculation = MerkleBlock.reduceFrom(
            this.getTxCount(),
            this.getFlags(),
            this.getHashes(),
            leafCallback,
        ...
        if (calculation.hashes.length > 0) {
            throw new Error(
                'Invalid partial merkle tree, hashes left without consuming after parsing'
            );
        }
        if (calculation.flags.length >= 8 || calculation.flags.replace(/0/g, '') !== '') {
            throw new Error(
                'Invalid partial merkle tree, flags left without consuming after parsing'
            );
        }
        if (calculation.hash !== this.getMerkleRoot()) {
            throw new Error(
                "Invalid partial merkle tree, computed merkle root doesn't match expected merkle root"
            );
        }
    ...
    static reduceFrom(
        ...
        if (currentOffset * 2 + 1 < nextLevelWidth) {
            ...
            if (left === right) {
                throw new Error(
                    'Invalid partial merkle tree, found equal hashes for left and right branches'
                );
            }
        } else {
            right = left;
