...
FederateRunner.prototype._setupCustomConfig = function () {
    ...
    if (
        this.options.bitcoinPeer &&
        this.options.customConfig['federator.bitcoinPeerAddresses.0'] == null
    ) {
...
FederateRunner.prototype._buildArguments = function (customConfig) {
    const args = ['-cp', this.options.classpath, `-Drsk.conf.file=${this.options.configFile}`];
    ...
    if (this.options.forks.activationHeights) {
        ...
        for (const key of activationHeightsKeys) {
            args.push(
                `-Dblockchain.config.hardforkActivationHeights.${key}=${this.options.forks.activationHeights[key]}`
            );
            overrideMessage.push(`${key}=${this.options.forks.activationHeights[key]}`);
        }
        this.options.runnerStdOut.write(
            ` overriding hard fork activation: ${overrideMessage.join(', ')}\n`
        );
    }
...
FederateRunner.prototype.start = function () {
    ...
    find.file(/federate-node.+\-all\.jar$/, dir, function (files) {
        ...
        if (maxFileName != null && path.basename(maxFileName) !== path.basename(filename)) {
            runnerStdOut.write(
                'WARNING Federate classpath uses ' +
                    path.basename(filename) +
                    ' but there is a newer version: ' +
                    path.basename(maxFileName) +
                    '\n'
            );
        }
    });
    ...
    const futurePorts =
        portsNeeded === 0
            ? Promise.resolve([])
            : portUtils.findFreePort(20000, 20100, '127.0.0.1', portsNeeded);
