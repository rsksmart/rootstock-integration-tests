...
// The RPC port can accept connections slightly before bitcoind is ready to serve RPC calls, so retry briefly.
function createWalletWithRetry(btcTxHelper, retriesLeft) {
    retriesLeft = retriesLeft == null ? 20 : retriesLeft;
    return btcTxHelper.createWallet().catch((err) => {
        if (retriesLeft <= 0) {
            throw err;
        }
        return new Promise((resolve) => setTimeout(resolve, 500)).then(() =>
            createWalletWithRetry(btcTxHelper, retriesLeft - 1)
        );
    });
}
    ...
    return futurePorts.then((selectedPorts) => {
        ...
        return portUtils
            ...
            .then((r) => {
                ...
                return createWalletWithRetry(btcTxHelper).then(() => r);
            });
    });
};
