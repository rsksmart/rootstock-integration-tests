...
/**
 * Returns the RSK node host URL from an rsk transaction helper (suitable for getBridgeState, etc.).
 * Ensures the result has an http(s) scheme (e.g. "127.0.0.1:30001" -> "http://127.0.0.1:30001").
 * @param {object} rskTxHelper - RskTransactionHelper instance
 * @returns {string} URL with http(s) scheme
 */
const getHostUrl = (rskTxHelper) => {
  const host = rskTxHelper.rskConfig.hostUrl;
  if (!host || typeof host !== 'string') {
    return host;
  }
  const hostTrimmed = host.trim();
  if (hostTrimmed.startsWith('http://') || hostTrimmed.startsWith('https://')) {
    return hostTrimmed;
  }
  return `http://${hostTrimmed}`;
};
