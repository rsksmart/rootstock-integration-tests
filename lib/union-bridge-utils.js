...
const assertIncreaseUnionLockingCapVotedEventWasEmitted = (txReceipt, newLockingCap, authorizedMember) => {
  ...
  expect(foundIncreaseLockingCapEvent, `Expected to find event with name "${IncreaseUnionLockingCapVotedEvent}"`).to.not
    .be.undefined;
  expect(foundIncreaseLockingCapEvent.returnValues.newLockingCap).to.equal(newLockingCap);
  expect(foundIncreaseLockingCapEvent.returnValues.voter.toLowerCase()).to.equal(authorizedMember.toLowerCase());
};

const assertIncreaseUnionLockingCapExecutedEventWasEmitted = (txReceipt, newLockingCap) => {
...
const assertUnionTransferPermissionsExecutedEventWasEmitted = (txReceipt, requestEnabled, releaseEnabled) => {
...
const assertBridgeCallFailedEventWasEmitted = (txReceipt, unionResponseCode) => {
  const executedEventName = "BridgeCallFailed";
  const foundExecutedEvent = txReceipt.events[executedEventName];
  expect(foundExecutedEvent, `Expected to find event with name "${executedEventName}"`).to.not.be.undefined;
  expect(foundExecutedEvent.returnValues.unionResponseCode).to.equal(unionResponseCode);
};
