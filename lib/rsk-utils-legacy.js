...
const { getBridgeAbi } = require('./bridge-provider');
...
const getMaxBlockNumber = (rskClients) => {
    let maxBlockNumber;
    let result = Promise.resolve(0);
    for (const client of rskClients) {
        result = result
            .then(() => client.eth.getBlockNumber())
            .then((bn) => {
                maxBlockNumber =
                    maxBlockNumber == null || maxBlockNumber < bn ? bn : maxBlockNumber;
...
const waitForBlock = (
    rskClient,
    blockNumber,
    waitTime = waitForBlockAttemptTimeMillis,
    maxAttempts = waitForBlockMaxAttempts
) => {
            ...
            rskClient.eth
                ...
                .then((bn) => {
                    ...
                    if (attempts++ === maxAttempts) {
                        ...
                        reject(
                            new Error(
                                `Block number ${blockNumber} never reached, last seen was ${bn}`
                            )
                        );
                    }
                })
...
const sendTxWithCheck = (rskClient) => (method, check, fromAddress) => {
    const txReceiptPromise = method
...
const triggerRelease2 = async (rskClients, releaseCreatedCallback, releaseConfirmedCallback) => {
...
const triggerPegoutEvent = async (rskClients, releaseCreatedCallback, releaseConfirmedCallback) => {
...
const triggerReleaseWithMineFunction = async (
    rskClients,
    mineFunction,
    releaseCreatedCallback,
    releaseConfirmedCallback
) => {
    const rskClient = rskClients[0];
...
const sendFromCow = (rskClient) => (toAddress, amount) => {
    let initialAddressBalance;
    let cowAddress;
    ...
    return Promise.resolve()
        ...
        .then(() => {
            const txPromise = rskClient.eth.sendTransaction({
                from: cowAddress,
                to: toAddress,
                value: amount,
            });
...
const getBridgeEventInBlockAndRunAssertions = async (
    block,
    eventName,
    eventAssertions,
    rsk,
    ...
    const eventJson = abi.find((methods) => methods.name == eventName && methods.type == 'event');
    expect(eventJson, `Event with name "${eventName}" was not found in the abi`).to.not.be
        .undefined;
    const signature = rskClient.eth.abi.encodeEventSignature(eventJson);
    for (const tx of txs) {
        const txReceipt = await rskClient.eth.getTransactionReceipt(tx);
        if (txReceipt.to == rsk.getBridgeAddress()) {
            for (const log of txReceipt.logs) {
                if (log.topics[0] == signature) {
                    ...
                    await eventAssertions(decodedLog, rskClient, {
...
const getBridgeEventAndRunAssertions =
    (eventName, eventAssertions, rsk, maxPastBlocksToCheck) => async (rskClient, blocksDepth) => {
        let block = await rskClient.eth.getBlock('latest');
        if (blocksDepth) {
            block = await rskClient.eth.getBlock(block.number - blocksDepth);
        }

        const assertionRun = await getBridgeEventInBlockAndRunAssertions(
            block,
            eventName,
            eventAssertions,
            rsk,
        ...
        if (!assertionRun) {
            ...
            if (maxPastBlocksToCheck > blocksDepth) {
                return getBridgeEventAndRunAssertions(
                    eventName,
                    eventAssertions,
                    rsk,
                    maxPastBlocksToCheck
                )(rskClient, blocksDepth + 1);
            }
            throw new Error('Conditions not reached');
        }
    };

const getTransactionHashFromTxToBridge = async (functionName, rsk, rskClient) => {
    const abi = getBridgeAbi();
    const block = await rskClient.eth.getBlock('latest');
    const txs = block.transactions;
    const eventJson = abi.find(
        (method) => method.name == functionName && method.type == 'function'
    );
    expect(eventJson).to.not.be.undefined;
    const signature = rskClient.eth.abi.encodeFunctionSignature(eventJson);

    for (const txHash of txs) {
        const txDetail = await rskClient.eth.getTransaction(txHash);
