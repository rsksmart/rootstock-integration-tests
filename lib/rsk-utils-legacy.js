...
const sendFromCow = (rskClient) => (toAddress, amount) => {
    ...
    return Promise.resolve()
        ...
        .then(() => {
            let txPromise = rskClient.eth.sendTransaction({
                from: cowAddress,
                to: toAddress,
                value: amount,
            });
        ...
        if (txReceipt.to == rsk.getBridgeAddress()) {
            for (const log of txReceipt.logs) {
                if (log.topics[0] == signature) {
                    let decodedLog = rskClient.eth.abi.decodeLog(
                        eventJson.inputs,
                        log.data,
                        log.topics.slice(1)
                    );
...
let getBridgeEventAndRunAssertions =
    (eventName, eventAssertions, rsk, maxPastBlocksToCheck) => async (rskClient, blocksDepth) => {
        ...
        let assertionRun = await getBridgeEventInBlockAndRunAssertions(
            block,
            eventName,
            eventAssertions,
            rsk,
...
let getTransactionHashFromTxToBridge = async (functionName, rsk, rskClient) => {
    ...
    const txs = block.transactions;
    const eventJson = abi.find((method) => method.name == functionName && method.type == 'function');
