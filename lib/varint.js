class VarInt {
    constructor(value, offset = 0) {
        if (typeof value === 'number' || typeof value === 'bigint') {
            ...
            throw new TypeError('Invalid input: value should be a number or buffer');
        }
    }
...
const encodeOutpointValuesAsMap = (utxos) => {
    return utxos.reduce((map, utxo) => {
        map[utxo.valueInSatoshis] = Buffer.from(new VarInt(utxo.valueInSatoshis).encode()).toString(
            'hex'
        );
        return map;
    }, {});
